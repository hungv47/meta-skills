...
- **Implementation work** → `system-architecture` (design) or direct task execution. Agents-panel debates; it doesn't build.
- **Verifying existing code/output** → `/fresh-eyes`. Different shape — fresh-eyes runs ONE reviewer against an output; agents-panel runs N agents against a question.
...
- **Decomposing work after a decision is made** → `/task-breakdown`. agents-panel makes the decision; task-breakdown plans the execution.
