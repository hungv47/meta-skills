---
name: task-breakdown
description: "Decomposes a spec or architecture into buildable tasks with acceptance criteria, dependencies, and implementation order for AI agents or engineers. Produces `skills-resources/meta/tasks.md`. Not for clarifying unclear requirements (use discover) or designing architecture (use system-architecture). For code quality checks after building, see fresh-eyes."
...
2. **Artifacts on disk** — `architecture/system-architecture.md`, `skills-resources/meta/specs/*.md`, every `skills-resources/product/flow/*.md`
...
**Read order:**
1. Pipeline: `architecture/system-architecture.md`, `skills-resources/meta/specs/*.md`, `skills-resources/product/flow/*.md`.
...
**Write-back:** none. Task lists are project-specific — they live in `skills-resources/meta/tasks.md`, not the user-profile experience layer.
...
6. **Assembly** — merge into artifact format. Seed each task block with `**History:**` entry (`{{today}} · task-breakdown · created`). Save to `skills-resources/meta/tasks.md`.
...
7. Save to `skills-resources/meta/tasks.md`
...
**First-run artifact (all pending, saved to `skills-resources/meta/tasks.md`):**
...
Save to `skills-resources/meta/tasks.md` using the Task Format above.
