...
**Load when:** the runner reaches Step 6 (dry-run report) or Step 8 (final report). The orchestrator writes the rendered report to `.agents/skill-artifacts/meta/records/[YYYY-MM-DD]-cleanup-artifacts-<slug>.md` — dated, slug-suffixed, immutable per-run (lifecycle: snapshot; see `agent-skills/CLAUDE.md` §"Artifact Placement"). Reports accumulate as audit trail; do NOT overwrite prior reports.
...
provenance:
  ...
  config_sources:                               # runtime configuration sources (not traceable inputs)
    - skills-resources/experience/technical.md   # if exists — supplies excluded_paths
