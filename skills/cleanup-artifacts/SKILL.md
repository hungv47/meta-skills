...
description: "Audits and grooms the skills-resources/ artifact tree — classifies every file (KEEP/STALE/ORPHAN/LEGACY/EPHEMERAL), surfaces references and risk, and (in --apply mode) MOVES candidates to skills-resources/.archive/[date]/ behind explicit per-category confirmation. Never deletes. Use when skills-resources/ has gone junk-drawer, before a release, or when artifacts feel stale. Not for cleaning code (use code-cleanup) or machine state (use machine-cleanup)."
...
promptSignals:
  phrases:
    - "clean up skills-resources"
    - "clean up the artifacts"
    - "groom the artifact tree"
    - "archive stale artifacts"
    - "prune skills-resources"
    - "tidy skill outputs"
    - "skills-resources is a junk drawer"
    - "artifacts piling up"
  allOf:
    - [clean, artifacts]
    - [prune, skills-resources]
...
routing:
  ...
  lifecycle: snapshot
  produces:
    - skills-resources/meta/records/[date]-cleanup-artifacts-*.md
  consumes:
...
*Meta — Single-agent orchestration with built-in critic gate. Audits and grooms the project's `skills-resources/` tree, classifying every artifact and (with explicit per-category confirmation) MOVING stale/orphan/legacy/ephemeral files to a dated archive. Never deletes.*
...
1. **MOVE, never delete.** Even after explicit operator confirmation, the destructive action is `mv` into `skills-resources/.archive/[YYYY-MM-DD]/`. A separate `--purge-archive` flag (out of scope for v1) is the only path to actual deletion. This guarantees recoverability.
...
## Output
- `skills-resources/meta/records/[YYYY-MM-DD]-cleanup-artifacts-<slug>.md` — dated, slug-suffixed, immutable per-run record (lifecycle: snapshot; see `agent-skills/CLAUDE.md` §"Artifact Placement"). Use a kebab-case `<slug>` capturing scope/mode (e.g., `2026-05-08-cleanup-artifacts-dry-run-full.md`, `2026-05-08-cleanup-artifacts-apply-research.md`). Reports accumulate as audit trail; do NOT overwrite prior reports.
...
- **Standalone re-run:** any time `skills-resources/` count exceeds the operator's tolerance threshold
...
**Needed dimensions:** scope (full `skills-resources/` vs. subdir), mode (`--dry-run` vs `--apply`), staleness threshold (default 90d), excluded paths the operator wants untouched.

**Read order:**
1. Conversation context — usually the operator just signalled `skills-resources/` is messy; mode and scope often inferable.
2. Pipeline: `skills-resources/manifest.json` for current artifact inventory + freshness signals.
...
**Warm Start** (scope clear from invocation, e.g., "groom skills-resources before commit"):

```
Found:
- scope → "[full skills-resources/ | <subpath>]"
- artifact disk snapshot →
  ! `find skills-resources -name "*.md" -type f 2>/dev/null | wc -l | tr -d ' '` files on disk;
  ! `find skills-resources -name "*.md" -type f -mtime +90 2>/dev/null | wc -l | tr -d ' '` older than 90d
- manifest last touched →
  ! `git log -1 --format='%cr' skills-resources/manifest.json 2>/dev/null | grep . || echo 'untracked or no git history'`
...
```
cleanup-artifacts grooms the skills-resources/ tree — classifies every file
(KEEP/STALE/ORPHAN/LEGACY/EPHEMERAL), surfaces references and risk,
and (with --apply) MOVES candidates to skills-resources/.archive/.
...
```
1. Read skills-resources/manifest.json + walk disk under scope target
...
7. [--apply only] mv candidates to skills-resources/.archive/[date]/
...
- **Live references** — grep result count for path-string + basename + slug across `skills-resources/`, `brand/`, `research/`, `architecture/`. Zero is required to proceed without escalation.
...
(Files MOVE to skills-resources/.archive/2026-05-08/, never deleted.
...
2. **NEVER delete** — only `mv` to `skills-resources/.archive/[YYYY-MM-DD]/`. Archive structure mirrors source (e.g., `skills-resources/meta/records/foo.md` → `skills-resources/.archive/2026-05-08/skills-resources/meta/records/foo.md`).
...
|---|---|---|
| Treating `skills-resources/meta/records/` as ephemeral | These are dated, immutable snapshots (audit trail). Pruning by age alone destroys the trail. | Use the `EPHEMERAL pattern list` in `references/cleanup-rules.md`. Records are STALE-only, never EPHEMERAL. |
...
| Operating without `skills-resources/manifest.json` fresh | Stale manifest classifies wrong. | Run `bun meta-skills/scripts/manifest-sync.ts` first if manifest mtime > 1 day. |
...
|-----------|---------|-------------|
| scope | `skills-resources/` | Path to audit. Subpaths allowed; HARD-NEVER paths refused. |
...
**Operator:** "groom skills-resources — it's a junk drawer. Apply."

**Pre-Dispatch (warm):**

```
Found:
- scope → "skills-resources/" (full)
...
- 47 artifacts found under `skills-resources/`
- Classifications:
  ...
  - ORPHAN (3) — `skills-resources/meta/sketches/foo.md` referenced by no manifest entry; producing skill (`prioritize`) hasn't been run in this project
  - LEGACY (1) — `skills-resources/meta/specs/old-scope.md` with `status: superseded` in frontmatter
  - EPHEMERAL (1) — `skills-resources/.archive/2026-04-01/foo-candidates.md` (already archived; matches `-candidates.md` pattern but already in archive — KEEP, it's the archive)

**Runner Critic Gate:**

- Spot-check 5 random STALE/ORPHAN: grep path + basename + slug across `skills-resources/`, `brand/`, `research/`, `architecture/`
- 1 of 4 STALE files (`2026-02-15-fresh-eyes-prior-pass.md`) cited by `skills-resources/meta/roadmap.md` line 412 — LIVE REFERENCE
...
Critic gate FAILED — 1 of 9 STALE/ORPHAN candidates has a live reference:
  STALE: 2026-02-15-fresh-eyes-prior-pass.md
    referenced by: skills-resources/meta/roadmap.md:412
...
**Operator decides:** edits roadmap.md to drop the stale reference, re-runs `cleanup-artifacts --apply`. Critic passes; per-category prompts fire; operator confirms STALE+ORPHAN+LEGACY; runner moves 8 files to `skills-resources/.archive/2026-05-08/`; manifest-sync runs; report saved with status DONE.
...
|---|------|-------|-------|----------|------|--------|------------|
| 1 | skills-resources/meta/records/foo.md | STALE | 12KB | 2026-01-15 | 0 | archived | y |
| 2 | skills-resources/meta/sketches/bar.md | ORPHAN | 4KB | 2025-11-30 | 0 | declined | n |
...
...
| File | Description |
|------|-------------|
| `skills-resources/meta/records/[YYYY-MM-DD]-cleanup-artifacts-<slug>.md` | Per-run audit report (lifecycle: snapshot — dated, immutable). Reports accumulate as audit trail. |
| `skills-resources/.archive/[YYYY-MM-DD]/...` | Archive of moved files, mirroring source paths (created on `--apply` only). |
...
- **NEEDS_CONTEXT** — `skills-resources/manifest.json` missing or corrupt; cannot classify reliably. Recommend `bun meta-skills/scripts/manifest-sync.ts` first.
