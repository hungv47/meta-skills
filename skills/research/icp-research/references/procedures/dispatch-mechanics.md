...
**Quick ICP (Route A):** Skip decision-psychology-agent. Pain-analysis feeds directly to synthesis (with no `decision_psychology` input), then critic. Critic still runs all 7 gates but Gate 4 (Decision Psychology Specificity) automatically PASSes with an "empty per Route A" note.
...
### PASS
Artifact meets all 7 quality gates. Deliver synthesis output to `research/icp-research.md`. Run post-write side effects (below).

### FAIL
Critic returns:
- Which gate failed and what's wrong (per the 7-gate Quality Gate Criteria in `agents/critic-agent.md`).
...
1. **Write `research/icp-research.md`** with frontmatter (skill, version, date, status) + body per `format-conventions.md` Artifact Template.
2. **Write or update `research/product-context.md`** with the 8-section schema per `format-conventions.md` Step 0 Product Context. On re-run with prior context, update in place; on first run, create.
3. **Experience write-back** per `pre-dispatch.md` Write-back map:
   - Q1 (Product) → append to `experience/product.md` AND ensure `research/product-context.md` Product section reflects it (canonical mirror).
...
4. **Versioning:** On re-run, rename prior `research/icp-research.md` to `research/icp-research.v[N].md` and increment N. Preserve prior `research/product-context.md` similarly if structure changes; otherwise update in place.

**BLOCK does NOT trigger write-back.** Partial runs that fail at the synthesis layer (e.g., critic FAIL after 2 cycles + operator deems Known Issues too severe to ship) do not persist canonical state. The previous `research/icp-research.md` (if any) remains the source of truth.
...
**Next:** `campaign-plan`, `brand-system` (marketing-skills). Both consume `research/icp-research.md` + `research/product-context.md`.

**Foundational role:** Creates `research/product-context.md`, used by 13+ downstream skills across all 4 stacks (comms, strategy, prod, design). Run first for significantly better downstream output. The recommended starting point per `research-skills/CLAUDE.md`.
...
- **Skipping the critic gate on `--fast`.** `--fast` collapses critic to a single pass; it does NOT skip the critic entirely. Without the critic, the 7 quality gates are unenforced and Critical Gate 1 / 2 / 3 violations ship silently.
...
- **Skipping the post-write canonical mirror.** Q1 (Product) writes to BOTH `experience/product.md` AND `research/product-context.md`. Skipping the latter leaves downstream skills reading a stale or empty product-context, defeating the foundational role.
