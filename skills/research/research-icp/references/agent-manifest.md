...
| **C — Called by another skill** | Invoked by plan-campaign, create-brand, write-copy, etc. | Read context from caller's artifacts; check `research/icp-research.md` freshness — Fresh (<30 days) return existing; Stale (>30 days) warn caller + recommend re-run; Missing → run Route B |
