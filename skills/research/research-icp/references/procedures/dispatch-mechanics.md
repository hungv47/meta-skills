...
4. **Pass upstream artifacts by content, not path**: orchestrator reads `.forsvn/canonical/product/PRODUCT-CONTEXT.md` first, then includes relevant excerpts (product, buyer, problem, differentiator) in context. Sub-agents do NOT read artifacts directly — orchestrator curates.
...
### PASS
Artifact meets all 10 quality gates. Deliver synthesis output to `.forsvn/canonical/research/ICP.md`. Run post-write side effects (below).
...
1. **Write `.forsvn/canonical/research/ICP.md`** with frontmatter (skill, version, date, status, stack: research, review_surface: html, id: icp-research, type: canonical, keywords) + body per `format-conventions.md` Artifact Template.
2. **Write or update `.forsvn/canonical/product/PRODUCT-CONTEXT.md`** with the 8-section schema per `format-conventions.md` Step 0 Product Context. On re-run with prior context, update in place; on first run, create.
3. **Experience write-back** per `pre-dispatch.md` Write-back map:
   - Q1 (Product) → append to `experience/product.md` AND ensure `.forsvn/canonical/product/PRODUCT-CONTEXT.md` Product section reflects it (canonical mirror).
...
4. **Versioning:** On re-run, overwrite prior `.forsvn/canonical/research/ICP.md` in place and increment the integer `version:` (prior versions live in git history — no `.v[N].md` siblings under `canonical/`). Update `.forsvn/canonical/product/PRODUCT-CONTEXT.md` in place similarly.

**BLOCK does NOT trigger write-back.** Partial runs that fail at the synthesis layer (e.g., critic FAIL after 2 cycles + operator deems Known Issues too severe to ship) do not persist canonical state. The previous `.forsvn/canonical/research/ICP.md` (if any) remains the source of truth.
...
**Next:** `plan-campaign`, `create-brand` (marketing-skills). Both consume `.forsvn/canonical/research/ICP.md` + `.forsvn/canonical/product/PRODUCT-CONTEXT.md`.

**Foundational role:** Creates `.forsvn/canonical/product/PRODUCT-CONTEXT.md`, used by 13+ downstream skills across all 4 stacks (comms, strategy, prod, design). Run first for significantly better downstream output. The recommended starting point per `research-skills/CLAUDE.md`.
...
- **Skipping the post-write canonical mirror.** Q1 (Product) writes to BOTH `experience/product.md` AND `.forsvn/canonical/product/PRODUCT-CONTEXT.md`. Skipping the latter leaves downstream skills reading a stale or empty product-context, defeating the foundational role.
