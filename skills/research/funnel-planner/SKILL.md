...
1. Read `implementation-roadmap/canonical-paths.md` if present — verify output path matches canonical inventory (`.agents/skill-artifacts/meta/records/targets-*.md`).
...
- **Path:** `.agents/skill-artifacts/meta/records/targets-[YYYY-MM-DD].md` (one per initiative-set; re-run renames prior to `targets.v[N].md` and increments)
...
- **Consumed by:** `campaign-plan` (channel-level execution); downstream measurement (eval-loop dashboards when one exists); future `funnel-planner` re-runs (delta detection)
- **Cross-stack OUTPUT contract:** Target Table column schema + section order are load-bearing for downstream consumers — schema changes require atomic update of `campaign-plan` and `eval-loop` (per `anti-patterns.md` row "Cross-stack contract drift")
