...
- Single initiative
- Prior `.agents/skill-artifacts/meta/records/targets-*.md` exists for the same initiative
...
After the artifact is written to `.agents/skill-artifacts/meta/records/targets-[date].md`:
...
|---|---|---|
| `prioritize` (hard-gated upstream — produces the ranked initiative list) | **funnel-planner** | `campaign-plan` (consumes targets to plan channel-level execution); downstream measurement (eval-loop dashboards) when a loop exists for the initiative |
...
| "What channels should we run?" | `campaign-plan` (consumes funnel-planner output) |
| "Why did conversion drop?" | `diagnose` (root-cause, not forward target) |
| "What's our TAM?" | `market-research` |
| "Build the brief for this campaign" | `campaign-plan` → per-asset skills (short-form-brief, design-brief, etc.) |
