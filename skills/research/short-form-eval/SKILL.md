...
1. Read `implementation-roadmap/canonical-paths.md` if present — verify output paths match canonical inventory (`skills-resources/loops/[slug]/evals/` + `results.tsv`).
2. Read `.agents/manifest.json` — find the matching `short-form-research` catalog by topic+market; check freshness.
...
- **Path:** `skills-resources/loops/[slug]/evals/[YYYY-MM-DD]-cycle-N.md` (one file per cycle; single platform, single brief)
...
- **Side effects:** append row to `skills-resources/loops/[slug]/results.tsv` via `bun scripts/append-loop-result.ts`; call `bun scripts/manifest-sync.ts` after artifact write (both mandatory per `procedures/dispatch-mechanics.md`)
- **Consumed by:** future `short-form-research` re-runs (mine pattern-log entries to update catalogs); gap-gate analysis (eventual); operator audit
- **Cross-stack INPUT contract:** reads from `short-form-research` catalog at `catalog_path` — frontmatter (`sample_size_per_platform`, `mechanics_sources_verified`, `trend_signals_date`, `platform_mechanics_date` drive `catalog_freshness` derivation), §3 Per-Platform Findings (hook archetypes feed `hook-strength-agent`), §6 Recommendations for short-form-brief (the reference patterns the brief should have followed, feed `eval-runner-agent`). Schema drift on these breaks Layer-1 dispatch.
...
5. Write artifact to skills-resources/loops/[slug]/evals/[date]-cycle-N.md;
