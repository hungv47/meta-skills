...
- **The artifact IS the contract.** Output frontmatter + body section order are consumed by future `short-form-research` re-runs (pattern-log mining) and gap-gate analysis (eventual). Schema drift requires atomic update of downstream consumers — never silent.

## When NOT to use this skill

- **Pre-publish brief authoring** — use `short-form-brief` (marketing-skills). Different job; this skill scores what brief produced, not what brief should be.
- **Catalog discovery** ("what's working on TikTok right now?") — use `short-form-research`. Different lifecycle: research builds the reference; eval scores against it.
- **No matching catalog exists for post's topic+market** — BLOCKED, defer to `short-form-research` first. Eval against missing reference is meaningless.
- **No loop workspace exists** — defer to `eval-loop` to create the workspace, then run eval inside it.
