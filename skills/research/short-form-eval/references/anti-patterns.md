...
| Free-form pattern claims | Prose like "credential-flash hooks seem to work better" is unusable downstream by gap-gate or by future `short-form-research` re-runs. | Pattern-log entries MUST use the canonical 4-line shape: **Claim** / **Evidence** / **Refutability** / **Expiry**. Critic rubric #3 enforces. |
...
| Treating catalog missing as a soft warn | If no catalog matches the post's topic+market, scoring is fabrication. There's no reference to score against. | BLOCKED with recommendation to run `short-form-research` first. Don't best-effort against missing reference. |
...
| Cross-stack contract drift | Adding new frontmatter fields or body sections without updating downstream consumers (future `short-form-research` re-runs, gap-gate). | Output Artifact Structure is the cross-stack contract. Schema changes require atomic update of downstream consumers — flag to operator before changing. |
