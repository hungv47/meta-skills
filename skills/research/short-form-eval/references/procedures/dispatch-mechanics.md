...
1. Write artifact to `skills-resources/loops/[slug]/evals/[YYYY-MM-DD]-cycle-N.md`.
2. Append row to `skills-resources/loops/[slug]/results.tsv` via `bun scripts/append-loop-result.ts` (validates header schema; rejects schema drift).
3. Call `bun scripts/manifest-sync.ts` so `.agents/manifest.json` indexes the new cycle artifact.

Skipping step 2 breaks the loop's ledger; skipping step 3 makes the cycle invisible to future state-detection by `orchestrate-research` and `orchestrate-meta`. Both are mandatory.
...
- No platform-intel catalog exists → `short-form-research` first (BLOCKED if missing — eval against missing reference is meaningless)
...
- No loop workspace → `eval-loop` to create one first
