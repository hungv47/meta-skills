...
- **Loop slug** (positional arg) — existing marketing loop under `skills-resources/loops/[slug]/`
...
- **Loop slug doesn't resolve to existing workspace** → BLOCKED; recommend `eval-loop` to create the workspace first.
...
- **No matching catalog after Cold Start** → BLOCKED; defer to `short-form-research`.
