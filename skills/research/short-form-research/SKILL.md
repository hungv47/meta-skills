...
- **Path:** `.agents/skill-artifacts/research/short-form-research/[slug].md` (one artifact per topic+market+platform-set)
...
- **Consumed by:** `short-form-brief` (marketing-skills) per-asset, reads §6 Recommendations + frontmatter sample-size flags; `short-form-eval` (research-skills) cycle-N scorer, reads frontmatter + §3 Per-Platform Findings to score published posts against the catalog
