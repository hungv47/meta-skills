...
The catalog isn't a survey. It's a **catalog of bets**. The downstream consumer (`short-form-brief`) reads this artifact to choose specific recommendations per asset ("TikTok hook should be credential-flash archetype in 0–1.5s — 8/12 in this sample"), not generic advice ("strong hooks matter"). Specificity is the contract.
...
- **The artifact IS the contract.** Output artifact frontmatter + body section order are consumed downstream by `short-form-brief` (per-asset) and `short-form-eval` (cycle-N scoring against the catalog). Contract changes require atomic updates to both consumers — never silently drift the schema.
...
- **Static visual** (images, carousels, infographics) — use `design-brief`.
- **Per-asset brief** (specific hook, shot list, captions for one piece) — use `short-form-brief` (marketing-stack); it consumes this catalog as input.
- **Audience research** (who buys, why) — use `icp-research`. This skill assumes audience is known or will run with a cold-start hint.
- **Competitive landscape mapping** (TAM/SAM/SOM, competitor positioning) — use `market-research`. This skill mines patterns within platform feeds, not market dynamics.
