...
1. **Write `research/market-research.md`** with frontmatter (skill, version, date, status) + body per `format-conventions.md` Artifact Template.
...
3. **Versioning:** On re-run, rename prior `research/market-research.md` to `research/market-research.v[N].md` and increment N.

**No canonical mirror.** market-research's write-back goes to per-domain experience files only — there's no parallel canonical artifact to update (unlike icp-research, which mirrors Q1 to `research/product-context.md` as the cross-stack record).

**BLOCK does NOT trigger write-back.** Partial runs that fail at the cross-analysis or opportunity layer (e.g., critic FAIL after 2 cycles + operator deems `[REVIEWER NOTE]` annotations too severe to ship) do not persist scope state. The previous `research/market-research.md` (if any) remains the source of truth.

---

## Chain Position

**Previous:** none (Strategy track entry point) — `market-research` and `diagnose` are the two entry points and they're `parallel-with` each other.

**Next:** `prioritize` (consumes Top 3 Opportunities as ICE-score evidence), `icp-research` (Underserved Segments inform persona scoping). Both downstream.

Also consumed by: `system-architecture` (competitive context for product decisions), `campaign-plan` (positioning anchors for IMC plans), fundraising decks (sizing + competitive landscape).
...
| Situation | Defer to |
|---|---|
| Audience personas + pain + VoC | `icp-research` (research-skills) |
| Root cause of a specific metric gap | `diagnose` (research-skills) |
| Know the problem, need solutions | `prioritize` (research-skills) |
| Channel mix and calendar from market signals | `campaign-plan` (marketing-skills) |
