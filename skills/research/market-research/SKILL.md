...
2. Read `.agents/manifest.json` — find any prior `research/market-research.md` (re-run signal) and check `research/product-context.md` presence (consumes input — recommend `icp-research` first if missing).
...
**Previous:** none (Strategy track entry point) | **Next:** `prioritize`, `icp-research` (both downstream).
