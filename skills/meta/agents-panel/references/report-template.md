...
**Load when:** standalone invocation (the operator typed `/agents-panel` directly, not invoked as a sub-routine). The orchestrator writes the rendered report to `.agents/skill-artifacts/meta/decisions/[YYYY-MM-DD]-agents-panel-<slug>.md` — dated, skill-prefixed, slug-suffixed, immutable per-run (lifecycle: decision). Decisions accumulate as audit trail; do NOT overwrite.
