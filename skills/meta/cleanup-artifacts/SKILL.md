...
description: "Audits and grooms the .agents/skill-artifacts/ artifact tree — classifies every file (KEEP/STALE/ORPHAN/LEGACY/EPHEMERAL), surfaces references and risk, and (in --apply mode) MOVES candidates to .agents/skill-artifacts/.archive/[date]/ behind explicit per-category confirmation. Never deletes. Use when .agents/skill-artifacts/ has gone junk-drawer, before a release, or when artifacts feel stale. Not for cleaning code (use code-cleanup) or machine state (use machine-cleanup)."
...
promptSignals:
  phrases:
    ...
    - "prune .agents/skill-artifacts"
    - "tidy skill outputs"
    - ".agents/skill-artifacts is a junk drawer"
    - "artifacts piling up"
  allOf:
    - [clean, artifacts]
    - [prune, .agents/skill-artifacts]
  ...
  anyOf:
    ...
    - "stale artifacts"
    - "archive artifacts"
    - ".agents/skill-artifacts cleanup"
  noneOf:
    - "code review"
    - "refactor"
    - "machine cleanup"
...
routing:
  ...
  lifecycle: snapshot
  produces:
    - .agents/skill-artifacts/meta/records/[date]-cleanup-artifacts-*.md
  consumes:
...
```
Found:
- scope → "[full .agents/skill-artifacts/ | <subpath>]"
...
The two `` ! `<cmd>` `` lines are inline shell interpolation (canonized in `CLAUDE.md` § "Inline shell interpolation"). When `/cleanup-artifacts` is invoked as a slash command, Claude Code substitutes the command output before the LLM sees the prompt — so the warm-start lands with real counts. This interpolation fires ONLY from SKILL.md slash invocation; it does NOT fire when the body is Read from a ref. That's why the Warm Start template stays in body and only Cold Start moves to a ref.
...
|-----------|---------|-------------|
| scope | `.agents/skill-artifacts/` | Path to audit. Subpaths allowed; HARD-NEVER paths refused. |
