...
2. Read `architecture/system-architecture.md` + `.agents/skill-artifacts/meta/specs/*.md` + every `.agents/skill-artifacts/product/flow/*.md`. If none exist AND conversation has no source → recommend `/discover` or `/system-architecture` first.
3. Read existing `.agents/skill-artifacts/meta/tasks.md` if present — re-run behavior (additive vs snapshot) decision lives in [`references/task-format.md`](references/task-format.md) §"Re-run behavior".
4. No `experience/` dimension read — task-breakdown is project-specific, not user-profile-driven.

## Artifact Contract

- **Path:** `.agents/skill-artifacts/meta/tasks.md` (single file, edited in place across runs)
...
6. **Assembly** — merge into the artifact format per [`references/task-format.md`](references/task-format.md) [PROCEDURE]. Seed each task block with `**History:**` entry (`{{today}} · task-breakdown · created`). Save to `.agents/skill-artifacts/meta/tasks.md`.
