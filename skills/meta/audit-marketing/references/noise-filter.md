...
**Scanner-severity mapping (audit-marketing).** The deterministic scanner emits `block | warn | nit`; map them onto the rubric above — **`block` → Blocker** (the report's top group; the tiny D-26 denylist subset, `mkt-claim-fabricated-precision` + the locked-brand-token analog, additionally carries the *denies-finalize* banner), **`warn` → Major**, **`nit` → Nit**. The report groups findings by the raw `block / warn / nit` token; the Blocker/Major/Nit labels stay the internal triage vocabulary. There is no Minor tier in marketing copy. The noise gate below fires at **≥5 findings and 0 `block`**.
