...
**Load when:** `.agents/skill-artifacts/meta/tasks.md` OR `.agents/skill-artifacts/meta/specs/*.md` exists in the project (orchestrator detects this in Step 1 of Execution). When present, the reviewer adds a scope check on top of the standard correctness/edge-cases/simplification/security/consistency review.
...
1. **Read `.agents/skill-artifacts/meta/tasks.md`** if it exists.
   - Are all tasks addressed by the diff?
   - Are there changes in the diff that don't map to any task?
2. **Read `.agents/skill-artifacts/meta/specs/*.md`** if any exist.
