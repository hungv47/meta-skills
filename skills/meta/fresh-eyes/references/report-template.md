...
**Load when:** Step 7 of Execution — writing the dated report to disk. Path: `.agents/skill-artifacts/meta/records/[YYYY-MM-DD]-fresh-eyes-<slug>.md` (dated, slug-suffixed, immutable per-run; lifecycle: snapshot). Reports accumulate as audit trail — never overwrite a prior report. Operator prunes via `cleanup-artifacts` when needed.
...
provenance:
  ...
  config_sources:
    - .agents/skill-artifacts/meta/records/learned-rules.md  # if any rules applied
