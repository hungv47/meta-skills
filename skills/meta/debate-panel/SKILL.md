...
`/debate-panel "Should we use a monorepo or polyrepo?"` — runs full debate or poll session, writes report to `.forsvn/artifacts/meta/decisions/`.

### 2. Sub-routine (another skill invokes mid-flow)

Callers (typical: `discover`, `prioritize`, `architect-system`) frame a specific decision and invoke debate-panel for the synthesis. Inline result, no disk write.
...
- **Consumed by:** operator (decision audit trail). No machine consumer in v1. Future skills MAY read prior decisions for precedent (e.g., `prioritize` checking past `debate-panel` outcomes for similar choices) — none do today.
...
Standalone or sub-routine. Typical callers when sub-routined: `prioritize`, `architect-system`, `discover`.
