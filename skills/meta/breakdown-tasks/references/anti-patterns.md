...
| Re-running task-breakdown to implement | task-breakdown is a planner, not a runner | After tasks.md ships, a fresh session (Claude Code, coding agent, human) reads execution-protocol.md and works tasks. Don't re-invoke breakdown-tasks. |
...
| Skipping system-architecture when stack is undefined | Tasks reference moving targets; round-2 critic FAIL is inevitable | Defer to /architect-system. Decomposition needs architectural decisions to anchor on. |
