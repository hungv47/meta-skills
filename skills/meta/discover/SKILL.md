...
When mode is `plan-review`, the user picks one of four sub-modes upfront via `AskUserQuestion` — SCOPE EXPANSION / SELECTIVE EXPANSION / HOLD SCOPE / SCOPE REDUCTION — once, locked for the session. The mode sets review posture and **locks the Step 7 Verdict vocabulary**. Full framework — postures, when-to-recommend signals, mode-pick mechanics, the load-bearing equal-weight rule, and the mode→verdict mapping: [`references/procedures/plan-review-modes.md`](references/procedures/plan-review-modes.md) [PROCEDURE].
