...
2. Read `.forsvn/index/manifest.json` + `.forsvn/index/artifact-index.md` (foundation files for meta skills).
3. Read `.forsvn/experience/technical.md` for any prior `cleanup-artifacts excluded paths`.
4. If `.forsvn/index/manifest.json` is missing or stale (>1d) → NEEDS_CONTEXT (don't fabricate); recommend `bun scripts/manifest-sync.ts`.
...
2. Pipeline: `.forsvn/index/manifest.json` for current artifact inventory + freshness signals.
