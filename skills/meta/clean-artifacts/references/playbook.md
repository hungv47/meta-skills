...
Every skill in the stack produces artifacts: briefs, audit reports, eval cycles, sketches, decisions. Over weeks, `.forsvn/artifacts/` accumulates files from skills the operator ran once and never touched again, eval loops that wound down, briefs that were superseded by later work. The pile grows past the point where the operator can tell, at a glance, what is still load-bearing and what is dead weight.
...
**MOVE, never delete.** The destructive action is `mv` into `.forsvn/artifacts/.archive/[YYYY-MM-DD]/`. Recoverable. Auditable. The operator who realizes "wait, I needed that" two weeks later finds it in the archive. A separate `--purge-archive` flag (out of scope for v1) is the only path to actual deletion.
...
- **HARD-NEVER is non-negotiable.** Canonical paths (`brand/`, `research/`, `architecture/`), submodules, session anchors (`tasks.md`, `roadmap.md`), the manifest, and `.forsvn/experience/` are never touched regardless of mtime or manifest state. Even on explicit operator request inside this skill. Operators can move them manually outside the skill if they really want to.
