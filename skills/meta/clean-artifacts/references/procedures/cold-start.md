...
```
cleanup-artifacts grooms the .forsvn/artifacts/ tree — classifies every file
(KEEP/STALE/ORPHAN/LEGACY/EPHEMERAL), surfaces references and risk,
and (with --apply) MOVES candidates to .forsvn/artifacts/.archive/.
Never deletes. Before I scan:

1. **Scope** — pick one:
   - full (.forsvn/artifacts/ — most common)
...
4. **Excluded paths** — anything off-limits even if it looks stale?
   (Persisted to .forsvn/experience/technical.md for future runs.)
...
| Q | File | Key |
|---|---|---|
| 4. Excluded paths | `.forsvn/experience/technical.md` | `Technical — cleanup-artifacts excluded paths` (durable across runs) |

Scope, mode, threshold are run-specific — NOT persisted.

## Defaults if operator answers partially

- Scope omitted → `.forsvn/artifacts/` (full)
- Mode omitted → `--dry-run`
- Threshold omitted → 90 days
- Excludes omitted → use whatever is already in `.forsvn/experience/technical.md` (empty list if none)
