...
- Route to `lp-brief` when the next action is a page revision or new test brief.
- Route to `copywriting` when the next action is only headline, CTA, or section copy variation.
- Route to `design-brief` when a single visual/proof asset needs specification.
- Route to `campaign-plan` when traffic/source mismatch is the dominant issue.
- Route to `eval-loop` when the metric contract, baseline, or guardrails need redefinition.
