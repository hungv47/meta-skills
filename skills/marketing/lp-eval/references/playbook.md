...
This skill is the canonical producer of `skills-resources/loops/[slug]/evals/[date]-cycle-N.md`. Side effects: appends one row to `results.tsv` via `scripts/append-loop-result.ts`; updates `learnings.md` only for high-confidence keep/discard lessons reusable beyond this exact page state; runs `manifest-sync` after writing.
...
3. **Scoring without a loop.** Critical Gate 1 requires an existing `skills-resources/loops/[slug]/program.md` + `context.md`. If absent → NEEDS_CONTEXT, recommend `/eval-loop`. lp-eval does NOT scaffold loops. Critic Hard Fail #1 enforces.
...
5. **Scope drift — evaluation becomes redesign.** Critical Gate 6 prevents lp-eval from redesigning. Recommendations route next work; actual page brief/revision goes to `lp-brief`, execution artifacts to appropriate content/design/build workflow. Critic dimension "Boundary Control" enforces.
...
Evaluation does NOT redesign. lp-eval recommends next changes and routes work; actual page revision goes to `lp-brief` (inside the same loop), and execution artifacts go to appropriate content/design/build skills. The boundary is load-bearing — confusing eval with redesign produces unmeasured changes that break the loop's measurement chain.
...
1. Write eval artifact at `skills-resources/loops/[slug]/evals/[date]-cycle-N.md`
...
- **No existing eval-loop workspace.** Use `/eval-loop` first to scaffold `program.md` + `context.md` + `results.tsv` + strategy/execution/evals subdirs.
- **Need the next page brief or redesign.** Use `lp-brief` — that's construction-time architecture, not post-launch scoring. lp-eval routes recommendations TO lp-brief but does not produce briefs.
- **The issue is channel strategy, not landing-page evidence.** Use `campaign-plan` — channel mix lives there.
...
- **Want to redesign the page based on the eval.** lp-eval recommends; `lp-brief` redesigns. Run lp-brief separately after lp-eval lands its verdict.
