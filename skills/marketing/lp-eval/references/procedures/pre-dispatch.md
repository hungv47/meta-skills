...
| Condition | Action |
|---|---|
| `skills-resources/loops/[slug]/program.md` AND/OR `context.md` absent | Return NEEDS_CONTEXT immediately; recommend `/eval-loop` (do NOT proceed to Cold Start) |
...
1. `skills-resources/loops/[slug]/program.md` — primary metric definition + guardrails + loop hypothesis
2. `skills-resources/loops/[slug]/context.md` — baseline assumptions + loop history + traffic-source notes
3. `skills-resources/loops/[slug]/results.tsv` — prior cycles' rows + resolve cycle number (`last cycle + 1`)
4. Latest files in `skills-resources/loops/[slug]/strategy/`, `execution/`, and `evals/` — what was changed this cycle + prior eval verdicts
...
**If the loop itself does not exist, return `NEEDS_CONTEXT` and recommend `/eval-loop` instead of asking the rest.** Do NOT scaffold loops from inside lp-eval.
...
**None.** lp-eval does NOT write to `skills-resources/experience/`. Persistent loop state lives in:
