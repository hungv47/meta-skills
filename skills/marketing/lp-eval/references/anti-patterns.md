...
**Pattern:** User invokes lp-eval on a slug that has no `skills-resources/loops/[slug]/program.md`. lp-eval tries to score anyway.
...
**Instead:** Critical Gate 1 fires BEFORE any other work — if `program.md` and `context.md` do not exist, return NEEDS_CONTEXT immediately and recommend `/eval-loop`. lp-eval does NOT scaffold loops.

**Owned by:** Critical Gate 1 (existing eval loop required) + Critic Hard Fail #1 (no existing `skills-resources/loops/[slug]/program.md`).
...
**Pattern:** User invokes lp-eval directly without running `/eval-loop` first. Loop directory doesn't exist or is empty.
...
**Instead:** lp-eval returns NEEDS_CONTEXT immediately, recommends `/eval-loop`. The eval-loop skill owns loop scaffolding; lp-eval owns post-launch evidence inside an existing loop.

**Owned by:** Orchestrator (Pre-Dispatch hard gate — fires BEFORE Cold Start questioning) + Critical Gate 1 + Critic Hard Fail #1 + Responsibility Split (`/eval-loop` owns loop setup).
...
**Pattern:** User invokes `humanize` or `vn-tone` on the eval artifact ("make the diagnosis sound less robotic"). Polish chain runs on what is supposed to be a structured operational document.
...
- Sibling coordination: lp-brief (`marketing-skills/skills/lp-brief/`) for construction-time architecture; eval-loop (`skills/meta/eval-loop/`) for loop scaffolding + durable learning ledger.
