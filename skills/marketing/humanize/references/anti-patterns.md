---
title: Humanize — Anti-Patterns Catalog
lifecycle: canonical
status: stable
...
These patterns apply across the marketing stack — humanize, as the EN polish-chain endpoint, sees them when invoked by upstream skills (Route C). Enforced via Pre-Dispatch wiring + critic verification + cross-skill contract.
...
**Problem:** Operator requests `detector_mode: pangram` but no `PANGRAM_API_KEY` is configured. humanize completes the polish, then writes `detector_status: pangram_pass` without actually running the classifier (because Pass 1 of the proxy checklist seemed clean). Frontmatter LIES about classifier verification.
