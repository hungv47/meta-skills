---
title: Humanize — Dispatch Mechanics
lifecycle: canonical
status: stable
...
3. **Resolve file paths to absolute** — replace relative paths with absolute paths rooted at this skill's directory. Example: `references/ai-patterns.md` becomes `/Users/you/skills/humanize/references/ai-patterns.md`. Tell the agent: "Read the reference file at [absolute path] for domain knowledge."
...
4. Return humanized output to the calling skill (no standalone artifact file written by humanize in Route C — calling skill embeds polished text in its own artifact)
