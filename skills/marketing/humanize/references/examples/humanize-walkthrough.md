---
title: Humanize — End-to-End Walkthrough (Route B, blog post)
lifecycle: canonical
status: stable
...
# Humanize — End-to-End Walkthrough

**Load when:** an operator (human or agent) wants a complete trace of how a Route B (full humanize) polish gets produced — Pre-Dispatch → Layer 1 parallel diagnosis → user checkpoint → Layer 2 sequential pipeline → critic PASS → deliver.
...
- **Operator invocation:** `/humanize` with paste of the original text.
...
# Humanized: SaaS Onboarding Blog Post
...
Same scenario, operator invokes `/humanize --fast`:
...
- Returns polished text + metadata to cold-outreach (no standalone artifact file written by humanize in Route C — cold-outreach embeds polished text in its own artifact)
- cold-outreach's artifact frontmatter records `polish_chain_applied: humanize`, `humanize_quality_score: 37/50`, `humanize_detector_status: proxy_pass`, `humanize_protected_tokens_preserved: true`
