---
title: Humanize — Format Conventions
lifecycle: canonical
status: stable
...
Frontmatter `date:` field uses ISO 8601 (`YYYY-MM-DD`) for machine-readable consistency with the rest of the artifact ecosystem. Content dates inside the humanized text follow the source convention (don't reformat dates inside quoted passages or named-entity references).
...
1. `## Humanization Summary` (10-row table: Words original / Words humanized / Compression / AI patterns found / AI patterns fixed / Hard Tells remaining / Soft Tells remaining / Quality score / Detector status / Protected tokens preserved)
2. `## Change Log` (4-column table: Location / Original / Change / Rule)
3. (Humanized content body — H2 sections from the original preserved as-is, no re-titling)

On re-run: rename existing artifact to `[slug].humanized.v[N].md` and create new with incremented version (`vN+1`).
