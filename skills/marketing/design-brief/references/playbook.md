...
1. **Hard-gate on brand artifacts BEFORE questioning.** No BRAND.md + DESIGN.md = NEEDS_CONTEXT, recommend `brand-system`. Cold-start questioning happens AFTER the gate clears — never before, even if the user is asking for "a quick IG post."
2. **Never invent tokens.** Every color, font, spacing token traces to DESIGN.md. If DESIGN.md doesn't cover what's needed (e.g., illustration style for an asset DESIGN.md never anticipated), flag it in the brief — don't guess. Expanding DESIGN.md is `brand-system`'s job.
...
- **Brand identity definition** → use `brand-system`. Design-brief CONSUMES brand/BRAND.md + brand/DESIGN.md; it does not produce or modify them. Inventing palette / typography / motion tokens for an asset = anti-pattern.
- **Whole-page redesigns** → use `lp-brief`. Design-brief is per-asset; lp-brief is per-page (with per-asset slots it can route to design-brief).
- **Writing the copy that goes IN the asset** → use `copywriting`. Design-brief composes the asset around copy; copywriting writes the headline / body / CTA.

## When NOT to use

- BRAND.md or DESIGN.md is missing. Hard-gate returns NEEDS_CONTEXT; route to `brand-system` first.
- The request is to render an asset, not to brief one. design-brief is brief-only; rendering happens downstream.
- The user wants an entire page or interface. Use `lp-brief` (per-page) or coordinate with the design system rather than producing isolated per-asset briefs.
- The user wants brand identity work (logo, palette, voice). Use `brand-system`; design-brief produces no brand decisions, only applies them.
- The asset is a wireframe / interface mockup. design-brief targets graphic assets, not UI flows. Use `user-flow` for screen mapping; use `system-architecture` for UI architecture.

## What it pulls from elsewhere

- `brand/BRAND.md` — voice, archetype, sacred elements (from `brand-system`)
- `brand/DESIGN.md` — palette, typography, surface language, motion (from `brand-system`)
- `brand/ASSETS.md` — per-platform production inventory; pre-fills format/dimensions if asset matches a row, auto-ticks checkbox on completion (from `brand-system`)
- `.agents/skill-artifacts/mkt/lp-brief/[slug]/asset-slots/[slot-id].md` — slot spec when brief is for an LP asset (from `lp-brief`)
- `.agents/skill-artifacts/mkt/content/[slug].copy.md` — copy used IN the asset (from `copywriting`)
- `.agents/skill-artifacts/mkt/campaign-plan.md` — campaign context, channel placement, awareness stage (from `campaign-plan`)
- `research/icp-research.md` — audience visual preferences (from `icp-research` in research-skills)
...
The brief at `.agents/skill-artifacts/mkt/design-briefs/[slug].md` is consumed by:
...
- **`lp-brief`** — when a landing-page asset slot needs a per-asset brief, lp-brief invokes design-brief on the slot
