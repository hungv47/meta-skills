...
4. **Pass upstream artifacts by content:** orchestrator reads `.agents/skill-artifacts/` files itself, includes relevant excerpts inline.
...
3. **Copy anchor** — resolve in-asset copy from copywriting artifact OR interview user (per copy-anchor-agent's role).
...
**Previous:**
- `brand-system` — REQUIRED. Hard gate on `brand/BRAND.md` + `brand/DESIGN.md`.
- `lp-brief` — OPTIONAL. When design-brief is invoked from a landing-page asset slot, lp-brief passes the slot spec via `.agents/skill-artifacts/mkt/lp-brief/[slug]/asset-slots/[slot-id].md`.
- `copywriting` — OPTIONAL. When in-asset copy is supplied separately via `.agents/skill-artifacts/mkt/content/[slug].copy.md`.
...
- `lp-brief` consumes design-brief output back when the brief was for a landing-page slot.
...
- **"Design our brand identity" / "Create our brand"** → defer to `brand-system`. design-brief consumes brand artifacts; it does NOT produce them. Sequence: `brand-system` first, then design-brief per asset.
- **"Write the headline for this ad"** → defer to `copywriting`. design-brief composes the asset around copy; it does NOT write headlines or CTAs. Sequence: `copywriting` first or in parallel, then design-brief consumes the copy artifact.
- **"Redesign this landing page"** → defer to `lp-brief`. design-brief is per-asset; lp-brief is per-page. lp-brief can route to design-brief per asset slot when the page-level architecture is decided.
