...
- `.agents/skill-artifacts/mkt/content/blog-async-launch.copy.md` present (headline: "Stop managing chaos, start leading progress.")
- `.agents/skill-artifacts/mkt/campaign-plan.md` present (PLG, async-first launch, 60-day window, Pillar 1 "The meeting tax")
...
- copy → "Stop managing chaos, start leading progress." (from copywriting artifact)
...
Orchestrator:
1. Writes `.agents/skill-artifacts/mkt/design-briefs/og-blog-2026-05-18-async-launch.md` with the assembled brief.
...
5. If cycle 2 FAIL → deliver as DONE_WITH_CONCERNS with critic notes in artifact frontmatter; user can manually iterate or invoke `brand-system` to enrich DESIGN.md with explicit photography-style guidance.

---

## --fast variant snippet

Same setup. User invokes `/design-brief --fast`:
...
When `lp-brief` is producing a landing-page brief and needs a per-asset brief for the hero slot:

1. lp-brief invokes design-brief with: "Need brief for hero asset slot, image-gen route preferred, slot spec at `.agents/skill-artifacts/mkt/lp-brief/async-launch-lp/asset-slots/hero.md`."
...
6. Artifact written at `.agents/skill-artifacts/mkt/design-briefs/lp-async-launch-hero-2026-05-18.md` with `consumed: [.agents/skill-artifacts/mkt/lp-brief/async-launch-lp/asset-slots/hero.md]` in frontmatter.
