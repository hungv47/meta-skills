...
The schemas in this file are **cross-stack contracts**. The frontmatter `critic_score` + `critic_verdict` fields feed `humanize` / `vn-tone` polish-chain consumers and `eval-loop` ledger ingestion. Renaming a field or reordering body sections requires atomic update of downstream consumers per `anti-patterns.md` row "Cross-stack contract drift."
...
```
.agents/skill-artifacts/mkt/copy/[platform]-[YYYY-MM-DD]-[slug].md
```
...
- `brief_source` — either a path (`./.agents/skill-artifacts/mkt/short-form-brief/<slug>/brief.md`) OR an inline topic string. Tracks provenance.
...
- `polish_chain_applied` — `none` (default), `humanize`, or `vn-tone`. Set by the polish-chain runner; copywriter-agent leaves it `none`.
...
3. Polish skill updates frontmatter: sets `polish_chain_applied: humanize` (or `vn-tone`). Does NOT change `critic_score` or `critic_verdict` (those are the pre-polish baseline; polish-pass scoring would require re-running the critic, which is out of polish-chain scope).
