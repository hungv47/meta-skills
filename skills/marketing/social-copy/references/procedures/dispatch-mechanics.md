...
4. [Optional] Polish chain
   ...
   polish_chain=vn-tone    → route to vn-tone skill (terminal pass)
...
1. **Write `.agents/skill-artifacts/mkt/copy/[platform]-[YYYY-MM-DD]-[slug].md`** with 13-field frontmatter + 6-section body per `format-conventions.md`.
...
- `--polish-chain vn-tone` → invoke `vn-tone` skill with the artifact path + target register (báo chí / semi-casual / bro / pop-marketing — operator answers register at vn-tone's own Cold Start). vn-tone rewrites Body + CTA, updates frontmatter, appends notes.
...
**Previous:** `short-form-brief` (when the brief locks platform / hook / audience / goal) OR `campaign-plan` (when the campaign plan declares the social cadence) OR none (greenfield invocation with Cold Start).

**Next:** `humanize` / `vn-tone` (polish chain, optional terminal pass) OR direct operator publish workflow (when polish_chain=none).

**Horizontal role:** social-copy is invoked at any stage of the marketing pipeline — after brief, after campaign-plan, or standalone. It does NOT have a foundational role (unlike icp-research / brand-system); it's a leaf-node producer in the pipeline.
...
| Cold-outreach DMs (email, LinkedIn, Twitter, iMessage) | `cold-outreach` |
