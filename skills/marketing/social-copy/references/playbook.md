...
- **Paid Meta / Google / LinkedIn ad copy.** That's `ad-copy` — audience-temp framing, claim substantiation, policy compliance, 7-dimension rubric, 3-variant aggregate. social-copy is organic / native only.
- **Full video brief with storyboard.** That's `short-form-brief` — hook + shot list + on-screen text + audio plan + caption + CTA + aspect + length for live-action OR motion-graphic production. social-copy generates the caption layer; short-form-brief owns the video.
- **Landing-page copy, headlines, taglines.** That's `copywriting` — horizontal across any surface (LP hero, section copy, CTA, social-proof block). social-copy is platform-native social-only.
- **Email subject lines + body, newsletter, cold-outreach DMs.** That's `email-copy` (deliverability, sequence logic) or `cold-outreach` (signal-based personalization, per-channel craft).
...
- **Multi-market in one invocation.** Re-invoke per market. Vietnamese-market copy routes through `vn-tone` via `--polish-chain vn-tone` as a terminal pass.
...
Multi-platform = re-invoke with a different `platform` argument. This is not editorial — Tier 1 hook archetypes are platform-specific (TikTok's POV Callout Cliffhanger doesn't exist on LinkedIn; LinkedIn's pattern-interrupt expectation is 1–2/100 vs TikTok's 3+/100). A single artifact that tries to cover multiple platforms produces compromise copy that's optimal for none. Same logic for single-market — Vietnamese register polish requires a terminal `vn-tone` pass that English-market copy doesn't need.
...
- `--polish-chain vn-tone` → routes critic-passed copy through `vn-tone` skill (Vietnamese register polish: báo chí, semi-casual, bro, pop-marketing). Required for Vietnamese-market copy.
...
Output artifact at `.agents/skill-artifacts/mkt/copy/[platform]-[date]-[slug].md` is consumed by:
...
- **`humanize` / `vn-tone`** (polish chain) — read the `## Body` and `## CTA` sections; rewrite in place; preserve frontmatter except adding `polish_chain_applied` field.
- **eval-loop** (when invoked inside a measurable initiative) — frontmatter `critic_score` + `critic_verdict` fields fed into `skills-resources/loops/[slug]/results.tsv` for keep/discard/watch/blocked decisions on social-copy variants.
