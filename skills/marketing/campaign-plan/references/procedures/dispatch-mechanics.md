...
4. **Pass upstream artifacts by content:** orchestrator reads `.agents/skill-artifacts/` files itself, includes relevant excerpts inline. Do not pass file paths and hope the agent reads them — the agent has limited tool access by design.
...
**Previous:** `icp-research` (creates `research/product-context.md` + `research/icp-research.md` — the canonical ICP record campaign-plan consumes).

**Next:**
- `lp-brief` (per landing page consuming the campaign's channel + angle assignment)
- `seo` (search-channel keyword + content execution)
- `cold-outreach` (mailbox-channel outbound execution, per-touch composition)
- `ad-copy` (paid Meta social-channel composition, per audience-temperature)
- `short-form-brief` (social-media-channel hero video brief)
- `funnel-planner` (sets per-channel numeric targets — CAC / LTV / per-channel CPL)

Campaign-plan is the **integration layer** between strategy (icp-research, prioritize) and execution (lp-brief, seo, cold-outreach, ad-copy, short-form-brief, funnel-planner). Downstream skills read `.agents/skill-artifacts/mkt/campaign-plan.md` for context.

---

## Re-run triggers

Re-invoke campaign-plan when:
...
- **"What should my CAC target be?" / "Per-channel CPL?" / "Conversion rate by channel?"** → defer to `funnel-planner`. Offer: "Run both — campaign-plan picks channels, funnel-planner sets per-channel numbers."
- **"Compare SEO pages" / "Keyword targeting" / "Site audit"** → defer to `seo`. Coordinate: "campaign-plan picks Search as a channel; `seo` turns that into a keyword tree + content structure."
- **"Landing page for this campaign"** → defer to `lp-brief`. Sequence: "campaign-plan first (so the page knows which channel + angle it serves), then lp-brief."
- **"Single ad" / "Single email" / "Single short-form video"** → defer to the specialist (`ad-copy`, `cold-outreach`, `short-form-brief`). Campaign-plan produces the integrated plan; specialists produce the tactical asset.
