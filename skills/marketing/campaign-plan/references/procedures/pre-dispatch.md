...
|---|---|---|---|
| 1 | **Product** (what it does, who pays) | Pillar pain-grounding, angle relevance, channel offer fit | `research/product-context.md`, `skills-resources/experience/product.md` |
| 2 | **Audience primary persona** (role + company size + 1-2 pains) | Pillar derivation, angle 3D framework, channel habitat selection | `research/icp-research.md`, `skills-resources/experience/audience.md` |
| 3 | **Campaign goal** (what 90 days needs to achieve) | Angle bank prioritization, channel weighting, timeline phase emphasis | `skills-resources/experience/goals.md` (Goals — current campaign focus) — usually Cold Start (campaign goals shift fast) |
| 4 | **Growth motion** (PLG / SLG / Hybrid) | Channel priority weighting (see table below) — **load-bearing for channel-agent** | `skills-resources/experience/business.md` (Business — growth motion) |
| 5 | **Duration + cadence** (e.g., 60 days, 3 posts/week) | Timeline shape, capacity-vs-cadence sanity check | `skills-resources/experience/goals.md` (Goals — campaign cadence) — usually Cold Start |
| 6 | **Constraints** (team size, budget tier, channels off-limits) | Cadence calibration, channel skip decisions, budget-type column in execution briefs | `skills-resources/experience/business.md` (Business — team + budget) |
...
1. **Pipeline artifacts** (highest priority):
   ...
   - `.agents/skill-artifacts/meta/sketches/prioritize-*.md` → strategic initiatives (optional, alignment only — campaign-plan does not gate on prioritize output)

2. **Experience substrate** (fills gaps pipeline doesn't cover):
   - `skills-resources/experience/{product,audience,business,goals}.md` for any dimension not covered by pipeline artifacts
...
4. **Detect intent mismatch:** if user input contains "CAC / LTV / target / conversion rate / per-channel number" language, surface the deference recommendation BEFORE dispatching: route to `funnel-planner` or run both in sequence (campaign-plan picks channels, funnel-planner sets per-channel numbers).
