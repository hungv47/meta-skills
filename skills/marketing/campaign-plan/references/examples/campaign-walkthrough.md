...
Reads `research/product-context.md` + `research/icp-research.md` + `skills-resources/experience/business.md`. Resolves 5 of 6 dimensions from artifacts; probes for campaign goal + cadence + constraints (the constraint detail above came from Cold Start question 5).
...
When `lp-brief` calls campaign-plan via Route C (read-then-fall-through-to-Route-B):

1. lp-brief invokes orchestrator with: "Need campaign context for landing page on async-first messaging."
2. Orchestrator reads `.agents/skill-artifacts/mkt/campaign-plan.md` — present + date 2026-05-18 (today) → return.
...
4. lp-brief writes its brief at `.agents/skill-artifacts/mkt/lp-brief/async-shipping-lp/brief.md` with `consumed: [.agents/skill-artifacts/mkt/campaign-plan.md]` in frontmatter.
