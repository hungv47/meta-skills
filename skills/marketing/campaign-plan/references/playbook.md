...
**Out of scope:**
- **Numeric targets (CAC, LTV, conversion rates, channel-CPL)** → use `funnel-planner`. Campaign-plan picks channels and cadence; funnel-planner sets the numbers each channel must hit.
- **Per-page conversion architecture** → use `lp-brief`. Campaign-plan picks landing pages as a channel; lp-brief designs their structure.
- **SEO keyword strategy + technical audit** → use `seo`. Campaign-plan can pick "Search engines" as a channel; seo turns that into a keyword tree + audit + execution plan.
- **Per-touch outbound composition** → use `cold-outreach`. Campaign-plan can pick "Mailbox" as a channel; cold-outreach composes the actual touches.
- **Per-asset short-form video brief** → use `short-form-brief`. Campaign-plan can include "Social media" with video as a format; short-form-brief turns each hero into a production-ready brief.
- **Per-ad creative for paid Meta** → use `ad-copy`. Campaign-plan can pick "Social media (paid)"; ad-copy composes the actual ads per audience-temperature.
...
- ICP research is missing AND the user can't provide enough context for a Cold Start to land. Recommend `icp-research` first; campaign-plan is downstream of it.
- The user wants a budget allocation across channels with target numbers attached. Pair with `funnel-planner` — campaign-plan picks the channels and weighting; funnel-planner sets the per-channel targets.
- The user wants a single-channel deep-dive (e.g., "build me a TikTok content strategy"). Use the specialist skill (`short-form-brief`, `social-copy`); campaign-plan's value is the 9-channel evaluation + integration, not a single-channel plan.

## What it pulls from elsewhere

- `research/product-context.md` — product positioning, accuracy constraints, unique mechanism, proof points (from `icp-research`)
- `research/icp-research.md` — primary persona, pains, habitats, VoC quotes, awareness levels (from `icp-research`)
- `.agents/skill-artifacts/meta/sketches/prioritize-*.md` — strategic initiatives (optional, alignment only; from `prioritize` in research-skills)
- `skills-resources/experience/{product,audience,business,goals}.md` — persisted answers from prior Cold Starts across the stack
...
## What downstream skills pull from campaign-plan

- `lp-brief` (Route C) — reads campaign context for hypothesis grounding
- `cold-outreach` (Route C) — reads campaign goal + ICP summary for personalization
- `seo` — coordinates on keyword targeting + content structure for the Search channel
- `ad-copy` (Route B) — reads ICP summary + offer + creative-format for paid social composition
- `short-form-brief` — reads campaign goal + pillar context for hero video angle
- `funnel-planner` — reads channel mix + cadence to set per-channel numeric targets

The artifact at `.agents/skill-artifacts/mkt/campaign-plan.md` is the shared reference these skills check before generating their own deliverables.

## History / origin

| Version | Date | Slot | Note |
|---|---|---|---|
| 8.0.0 → 8.0.0 | 2026-05-18 | v6 Phase 2 Wave 1 — marketing-stack slot 10/14 | Body 470 → ~210 (-55%) + 5 new refs (playbook + format-conventions + procedures/pre-dispatch + procedures/dispatch-mechanics + examples/campaign-walkthrough) + new anti-patterns.md (5 from body + 4 cross-cutting marketing-stack rows). Cross-stack contract preserved BYTE-IDENTICAL: 6 Critical Gates, 10-item Quality Gate (orchestrator-side; canonical 11-row + 9-row Rewrite Routing live in critic-agent.md), 6-agent Manifest, Routes A/B/C, Pre-Dispatch (6 needed dimensions + Warm/Cold templates + 5-row Write-back map + Growth Motion → Channel Priority table), Layer 1 + Layer 2 dispatch tables, Artifact Template (4-field frontmatter + Foundation/Pillars/Angle Bank/Channel Assignments/Channel Execution Briefs/Timeline/Launch Sequence sections), Completion Status 4-tier. Structural: nested `### Artifact Template` under new `## Artifact Contract` H2 wrapper per marketing-stack sibling-parity convention (matches ad-copy slot 9 + copywriting slot 8 + cold-outreach slot 7 + humanize slot 6 + vn-tone slot 5 + short-form-brief slot 4 + seo slot 3). |
