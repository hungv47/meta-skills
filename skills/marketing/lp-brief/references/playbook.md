...
lp-brief is the page-level orchestrator between strategy and design — it produces a campaign-grade landing-page brief (hypothesis, surface rhythm, section-by-section spec, asset slots, copy candidates, hand-off prompts) that a designer (or Claude Design, or coding agent) can execute without follow-up questions. The brief has a built-in conversion-principles gate (CP-01 → CP-13) that scores the brief at brief-time, so the page is *built* right rather than waiting for a separate heuristic audit pass after launch.
...
This skill is the canonical producer of `.agents/skill-artifacts/mkt/lp-brief/[slug]/brief.md` + always-emitted `handoff-implementation.md`. Per-asset generation prompts at `asset-slots/[slot-id].prompt.md` are written by downstream media-briefing skills (`design-brief` today; future motion-brief / 3d-brief / video-brief as they ship).
...
6. **Heuristic-review-as-CRO drift.** Treating a brief teardown as "post-launch optimization" — the brief is *construction-time best-practice gating*, not measurement-driven optimization. Anti-patterns explicitly flag this; the skill defers post-launch CRO to `lp-eval` inside an existing eval-loop.

The structural answer is the **dual-critic gate** (conversion-critic: 13 CP-IDs scoring 11 hard-gates + 2 NOTE-only; brand-voice-critic: 9 gates G1-G8b with sacred + envelope auto-FAIL). Both run in parallel at Layer 5; verdict is binary PASS/FAIL per critic; max 2 rewrite cycles; cycle-2 residual FAILs ship as DONE_WITH_CONCERNS with concerns pinned at top of brief.md (no silent failure).
...
The artifact IS the contract — a 14-section body schema with a 12-field frontmatter consumed by downstream coding agents, designers, and (indirectly via eval-loops) by `lp-eval` cycles. Schema changes require atomic update across all callers.
...
- **Approval Gate 3:** user sees the brief preview + critic merge. Approve → write artifacts to `.agents/skill-artifacts/mkt/lp-brief/[slug]/`. Revise X → re-dispatch named layer (1 cycle). Reject → save as `rejected.md`, exit BLOCKED.
...
- **Need post-launch CRO from real evidence.** Use `lp-eval` inside an existing `eval-loop`. lp-brief can read prior evals when producing the next brief, but does not pretend best-practice review is optimization.
- **Single visual asset spec, not whole page.** Use `design-brief` — per-asset brief, consumes BRAND.md + DESIGN.md.
- **No brand defined yet.** Use `brand-system` FIRST. Hard gate fires — lp-brief returns NEEDS_CONTEXT when BRAND.md or DESIGN.md is missing.
- **Need only headline variations.** Use `copywriting` — voice consumer, not page architect.
...
- [`conversion-principles.md`](conversion-principles.md) — canonical 13-principle conversion rubric (CP-01 → CP-13) read by section-spec-agent + conversion-critic-agent
