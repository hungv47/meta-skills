...
**Instead:** Build with the conversion rubric (CP-01 → CP-13) inside this brief; after launch, use real analytics/recordings/experiment evidence in `lp-eval` inside an existing eval-loop. Then feed the resulting eval into the next `lp-brief --rev=N` cycle.

**Owned by:** Orchestrator (`Skill Deference` block in body — "Need post-launch CRO from real evidence? → `lp-eval` inside an existing `eval-loop`") + Critical Gate 2 (do NOT skip conversion rubric) + brand-system-stack convention (lp-brief is brief-time; lp-eval is post-launch).
...
**Pattern:** User invokes lp-brief without running `brand-system` first. BRAND.md and DESIGN.md absent. lp-brief proceeds anyway by inventing brand context.
...
**Instead:** Hard gate fires BEFORE any cold-start questioning. Missing BRAND.md or DESIGN.md → return NEEDS_CONTEXT immediately, recommend `brand-system`. If either >60 days stale, warn and ask before proceeding. lp-brief is the canonical example of "hard gates before any cold-start question" pattern.
...
**Pattern:** lp-brief output gets used as input to a freshly-scaffolded eval-loop, but the eval-loop never gets a `program.md` or measurement baseline. Later, `lp-eval` is invoked → returns NEEDS_CONTEXT because the loop is empty.
...
**Instead:** When the brief is intended for an eval-loop cycle, scaffold the loop with `/eval-loop` FIRST (writes program.md, context.md, results.tsv). Then run lp-brief and copy the brief into the loop's `strategy/` directory. Then run lp-eval after launch. The Skill Chain section in brief.md step 5 explicitly names this: "[post-launch] collect analytics/recordings/experiment notes → run `lp-eval` inside the page's eval loop, then feed the resulting eval into next `lp-brief --rev=N`."

**Owned by:** Orchestrator (Skill Chain section formats the loop-coordination expectation in every brief) + lp-eval Critical Gate 1 (existing eval loop required — returns NEEDS_CONTEXT if `program.md` missing, recommends `/eval-loop`).
