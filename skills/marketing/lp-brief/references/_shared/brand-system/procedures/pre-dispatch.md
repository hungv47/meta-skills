...
2. **Experience:** `skills-resources/experience/{product, audience, brand, business}.md` for prior persisted answers.

If pipeline artifact `date` fields are >30 days old, warn and recommend re-running `icp-research` (Critical Gate 4).
...
After Cold Start answers resolve, write back to `skills-resources/experience/` so subsequent skills don't re-ask:
...
| Condition | Action |
|---|---|
| `research/product-context.md` AND `research/icp-research.md` both absent | STRONGLY recommend running `icp-research` first; ask user to confirm before proceeding (will produce generic archetype output if user insists) |
| Target platforms (Q6) unresolved | Hard-block; force enumeration via Cold Start Q6 + disambiguation rules |
| Product (Q1) describes a product the user does NOT own / is asking for "competitor analysis" of someone else's brand | Hard-block; brand-system produces brand identity for OWNED products, not competitive teardowns. Suggest `market-research` instead. |
| Pipeline artifacts >30 days old | Warn (Critical Gate 4) + recommend re-running `icp-research`; do NOT block (user may proceed with caveat) |
