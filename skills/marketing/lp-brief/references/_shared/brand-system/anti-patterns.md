...
**Pattern:** User invokes brand-system directly without running `icp-research` first. No audience data exists. Strategy-agent defaults to generic archetype + values + voice ("we serve customers who value quality and innovation").
...
**Instead:** Pre-Dispatch flags missing `research/product-context.md` + missing `research/icp-research.md` and STRONGLY recommends running `icp-research` first. If user proceeds anyway (e.g., greenfield with no customers yet), the Pre-Dispatch Cold Start gathers product + audience + competitive data inline; critic still scores against the same rubric.

**Owned by:** Orchestrator (Pre-Dispatch hard-recommendation when both research artifacts absent) + Critical Gate 4 (stale upstream data >30 days → recommend re-running `icp-research`).
...
**Pattern:** User invokes `humanize` or `vn-tone` on BRAND.md output ("make the voice section sound less AI-written"). Polish chain runs on what is supposed to be the canonical voice definition.
