...
1. **`brand/BRAND.md` AND `brand/DESIGN.md` must exist.**
   - Either missing → return **NEEDS_CONTEXT**, recommend `brand-system` (Route A for BRAND only when DESIGN.md exists; Route B for full design tokens when neither exists).
...
|---|---|---|---|
| 1 | **Asset type** (OG image / IG carousel / banner / hero / OOH / etc.) | concept-agent reference catalog selection + auto-detection of downstream route + platform-module lookup | Cold Start Q1 (always asked unless invoked from lp-brief with explicit slot) |
...
| 4 | **Copy/headline if any** | copy-anchor-agent placement + critic Typography dimension | `.agents/skill-artifacts/mkt/content/[slug].copy.md` if supplied; else Cold Start Q3 |
...
1. **Pipeline artifacts (hard-gate resolved):**
   ...
   - `.agents/skill-artifacts/mkt/lp-brief/[slug]/asset-slots/[slot-id].md` if invoked from lp-brief (carries slot spec)
   - `.agents/skill-artifacts/mkt/content/[slug].copy.md` if copy supplied separately (from copywriting)
   - `.agents/skill-artifacts/mkt/campaign-plan.md` for campaign context, awareness stage, channel placement
   - `research/icp-research.md` for audience visual preferences (optional but useful)

2. **Experience substrate:** `skills-resources/experience/{brand,goals}.md` for any durable constraint not covered by pipeline (e.g., legal disclaimer policies the user mentioned in a prior session).

3. **Detect intent mismatch BEFORE Cold Start:** if user input contains "make / create / render / generate" verb + asset noun (e.g., "make me an IG post"), surface deference recommendation: "design-brief produces the BRIEF for the asset; the BRIEF feeds Claude Design / Midjourney / Imagen / DALL·E / Pencil / Figma / a human designer who actually renders it. Proceed with brief, or are you looking for direct rendering?" Same for "redesign this page" → defer to `lp-brief`; "design our brand identity" → defer to `brand-system`. (Anti-pattern #11.)
...
If all 5 dimensions resolve (rare on first standalone run, common when invoked from lp-brief), skip even this probe and dispatch directly after one-line confirm.
