...
When `ad-copy` emits a brief, include `creative_format: dedicated | repurposed-UGC` and attach a ceiling warning to `repurposed-UGC` when the account's scale target is high.
