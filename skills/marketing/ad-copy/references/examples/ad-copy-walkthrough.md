...
- **Operator invocation:** `/ad-copy` with the 10 Cold Start dimensions filled inline.
...
4. No `.agents/skill-artifacts/mkt/campaign-plan.md` (standalone ad-copy, not part of broader campaign)
...
**Scenario:** `campaign-plan` is orchestrating a Q4 MealKit campaign across paid + organic + email. The paid mix includes BOTH warm (retargeting trial-finishers) AND cold (this MealKit scenario above). campaign-plan dispatches ad-copy TWICE (Route B is NOT stacked — one invocation per audience-temp).
