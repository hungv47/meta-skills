...
These are inherited from cold-outreach's voice-auditor + critic auto-fail list, scoped to ad copy. Run all of them across each variant. Any hit → either EDITS_APPLIED (if fixable in-place) or BLOCKED (if it requires composer rework).
