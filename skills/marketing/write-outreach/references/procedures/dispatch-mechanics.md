...
4. **Pass upstream artifacts by content, not path** — orchestrator reads `research/*.md` and `.forsvn/artifacts/mkt/*.md` FIRST, includes excerpts (VoC quotes, voice adjectives, pain language) in pre-writing. Sub-agents do NOT read artifact files directly.
