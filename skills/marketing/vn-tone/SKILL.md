...
| `.agents/skill-artifacts/mkt/content/[slug].md` | upstream | Optional — if polishing a prior artifact, extract register from frontmatter if present |
| `skills-resources/experience/brand.md` | (any skill) | Optional — `Brand — VN target register` key if user previously persisted a default |
...
Two routes — Route A for slash-command invocation, Route B for upstream skill auto-routing (e.g., `short-form-brief` / `social-copy` / `copywriting` / `ad-copy` etc. when `market = VN`).
...
- **Path (Route A):** `.agents/skill-artifacts/mkt/content/[slug].vn-tone.md`
