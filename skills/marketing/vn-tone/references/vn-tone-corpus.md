...
> Annotated corpus of Vietnamese writing registers, compiled from live samples scraped from representative publications and forums. Used by `vn-tone` agents to diagnose input register, set target register, and rewrite translated text so it reads naturally.
