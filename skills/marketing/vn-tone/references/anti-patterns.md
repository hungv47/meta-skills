...
**Problem:** A maintainer adds a new frontmatter field or body section to the vn-tone artifact without checking calling-skill consumers (`short-form-brief`, `social-copy`, etc. that read `polish_chain_applied` from vn-tone output). Schema drifts; downstream parsers (if any are added) break.
