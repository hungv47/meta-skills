...
1. **Pipeline artifacts:**
   ...
   - `.agents/skill-artifacts/mkt/content/[slug].md` if polishing a prior artifact (extract register from frontmatter if present)
2. **Experience:** `skills-resources/experience/brand.md` → register-mapping notes (only if user previously persisted `Brand — VN target register`)
...
|---|---|---|---|
| 1. Register | `skills-resources/experience/brand.md` | `Brand — VN target register` | ONLY if user explicitly wants this stable cross-run (e.g., "save this as my default"); routing-only otherwise (the register might be content-specific for that one polish, not the brand default) |
