...
- Brand tokens (`brand/BRAND.md`, `brand/DESIGN.md`) — palette, type, logo safe-zone, Signal-Lime <10% rule
...
- signal_lime_share: [<10% | overrun] (FORSVN brand rule — state-cue only)
...
- **Brand-fit is your job.** Check palette against the brand tokens, the Signal-Lime <10% rule, type stack, and logo safe-zone when tokens are supplied. An off-brand render is not a win.
...
- Did I check the brand tokens (palette, Signal-Lime <10%, type, logo) when supplied?
