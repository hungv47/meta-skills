...
**Three routes by intent.** Route A — Compose (first-touch or follow-up, slug `-t1`/`-t2`/`-t3`/etc.). Route B — Reply (user pastes inbound, classifier types it as one of 9 reply types). Route C — Called by another skill (`campaign-plan` invokes per touch in a broader campaign).
...
**Terminal humanize with specificity regression.** After critic PASS, humanize runs with `content-type: "short-outbound"` (Light strip, Full voice, 0-10% compression) + `protected_tokens` listing every named entity + number from the critic-approved draft. Post-humanize, re-run critic's Specificity dim only. Drops ≥2 OR any named entity/number absent → revert to critic-approved draft. **Automatic, not judgment** — AI-sounding cold email is the biggest failure mode, regression protects against humanize silently stripping the specificity anchor.
...
- **Humanize runs ONCE.** Running humanize twice strips specificity and drifts toward generic. Terminal pass is exactly one call with `protected_tokens` + post-humanize Specificity regression.
...
- **Need landing-page headline / tagline / ad CTA.** Use `copywriting` — writing craft, not outbound.
- **Multi-channel campaign across paid/owned/earned.** Use `campaign-plan` first, then this skill for the touches.
- **No persona, or ICP stale (>30d).** Use `icp-research` first — outbound without a persona produces generic outbound.
...
- [`procedures/dispatch-mechanics.md`](procedures/dispatch-mechanics.md) [PROCEDURE] — Routes A/B/C, Layer 1a + 1b two-stage strategy, Merge Step, Layer 2 sequential, single-agent fallback, critic gate + rewrite loop, Terminal humanize + specificity regression, Reply Route Agent Flow
...
- [`examples/cold-outreach-walkthrough.md`](examples/cold-outreach-walkthrough.md) [EXAMPLE] — end-to-end Route A walkthrough (services-sell, email touch 1 to a named target, signal-strength 4, O→P→P→A framework, critic PASS cycle 1 at 44/50, terminal humanize with `protected_tokens`, post-humanize regression passes)
...
- [`_shared/mode-resolver.md`](_shared/mode-resolver.md) — `--fast` behavior (this skill is `budget: deep`; `--fast` collapses Layer 1b parallel to sequential and skips post-humanize regression, but Critical Gates + Missing-Input Hard Blocks STILL enforced and Cold Start STILL fires when context missing)
