...
This walkthrough covers Route A (operator slash-command for a first-touch cold email to a named target). Route B (reply handling) follows the same Layer 2 mechanics but replaces Layer 1 with `reply-classifier` and composer with `reply-composer` — short snippet at the end. Route C (called by `campaign-plan`) follows Route A mechanics but skips the AskUserQuestion fallbacks (calling skill resolves mode + channel + per-touch context before invocation).
...
- **Operator invocation:** `/cold-outreach` with the 7 Pre-Dispatch dimensions filled inline.
...
3. `skills-resources/experience/product.md` → `Product — proof points` key has Ramp + Linear engagements already; Ramp is most-cited
4. No `.agents/skill-artifacts/mkt/campaign-plan.md` (standalone outreach, not part of a campaign)
...
Orchestrator writes 3 files to `.agents/skill-artifacts/mkt/cold-outreach/`:

- **`jane-acme-email-t1.md`** — final humanized text + frontmatter (`skill: cold-outreach`, `version: 1`, `date: 2026-05-18`, `status: done`, `channel: email`, `mode: services-sell`, `touch: 1`, `route: compose`, `critic_total: 44/50`)
