...
- **Operator invocation:** `/copywriting` for full landing page, with the 7-question Cold Start answers inline.
...
4. No `.agents/skill-artifacts/mkt/campaign-plan.md` (standalone landing page, not part of broader campaign)
...
Orchestrator writes single artifact to `.agents/skill-artifacts/mkt/content/statuszero-landing.copy.md`:

- Frontmatter: `skill: copywriting`, `version: 1`, `date: 2026-05-18`, `status: done`
...
- **Step 1:** Pre-Dispatch reads lp-brief's artifact at `.agents/skill-artifacts/mkt/lp-brief/statuszero/brief.md` — extracts audience + voice + Unique Mechanism + the one shift + traffic source per-section. No AskUserQuestion mid-flow.
...
- **Step 4:** Return annotated hero copy directly to lp-brief (NO standalone artifact file written by copywriting in Route C; lp-brief embeds copy + annotations in its own artifact at `.agents/skill-artifacts/mkt/lp-brief/statuszero/v1/brief.md`).
- **Completion Status: DONE.**
