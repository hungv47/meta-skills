...
These patterns apply across the marketing stack — copywriting is called by `campaign-plan` (Route C campaign touchpoints) and `lp-brief` (Route C per-section copy), and routes to `humanize` for terminal AI-pattern cleanup. Enforced via Pre-Dispatch wiring + critic verification + cross-skill contract.

### Calling skill drops voice + audience context in Route C

**Problem:** `lp-brief` dispatches copywriting Route C for a hero-section headline but passes only the brief, no voice adjectives, no audience VoC quotes, no Unique Mechanism. copywriting's Pre-Dispatch hits the Cold Start fallback and surfaces 7-question Cold Start mid-flow — jarring for the user who's mid-lp-brief flow.
