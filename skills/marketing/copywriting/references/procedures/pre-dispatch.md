...
2. **Pipeline artifacts:**
   ...
   - `.agents/skill-artifacts/mkt/campaign-plan.md` → angle + awareness stage
3. **Experience:** `skills-resources/experience/{audience,product,goals}.md` → prior copy shift + belief sequence + proof points
...
If `research/icp-research.md` / `research/product-context.md` `date:` is >30 days, warn and recommend re-running `icp-research` (soft gate — proceed with "stale ICP" header note).
...
| Q | File | Key |
|---|---|---|
| 2. Audience | `skills-resources/experience/audience.md` | `Audience — primary persona` (only if novel; pre-existing icp-research takes precedence) |
| 3. The one shift | `skills-resources/experience/goals.md` | `Goals — copy shift: [surface]` |
| 4. Unique proof | `skills-resources/experience/product.md` | `Product — proof points` |
| 5. Unique Mechanism | `skills-resources/experience/product.md` | `Product — unique mechanism` |
| 6. Belief sequence | `skills-resources/experience/goals.md` | `Goals — copy belief sequence: [surface]` |
...
- **VoC pain language** — verbatim quotes from icp-research.md
...
- **Unique proof + Unique Mechanism both missing AND no product-context.md.** NEEDS_CONTEXT. Recommend `brand-system` or `icp-research` to surface proof + mechanism before dispatching. Copy without proof = uncheckable claims = critic Competitive Uniqueness FAIL.
