...
- **Operator invocation:** `/short-form-brief --platforms tiktok,reels --brand-mode founder` with angle "We replaced standups with a bot. Output went up."
- **Existing context:**
  - `research/icp-research.md` exists (VN solo dev founder ICP)
  - `brand/BRAND.md` exists (founder archetype, semi-casual VN voice)
  - `.agents/skill-artifacts/research/short-form-research/dev-tools-vn.md` exists (8d old trends, 22d old mechanics — fresh)
  - `.agents/skill-artifacts/mkt/campaign-plan.md` does not match this angle
...
1. `.agents/skill-artifacts/research/short-form-research/dev-tools-vn.md` → found, trends 8d (fresh), mechanics 22d (fresh)
...
4. `skills-resources/experience/content.md` → prior brand_mode lock confirms founder
5. `.agents/skill-artifacts/mkt/campaign-plan.md` → no matching slug; no inheritance
...
Per (VN, founder): `vn-tone` Layer 2 on spoken-line section + full body.

`vn-tone` runs once (full-body pass), tightens hook verbal + VO direction + caption to semi-casual VN register (báo chí → semi-casual register shift), checks particles + pronoun consistency. Returns polished hero + polished variant. No re-dispatch of craft agents.
...
- `.agents/skill-artifacts/mkt/short-form-brief/standup-bot/brief.md` (hero, 14 sections per `format-conventions.md`)
- `.agents/skill-artifacts/mkt/short-form-brief/standup-bot/variants/reels.md` (variant, abbreviated 5-section set after "What Changed From Hero")
...
```
✓ Hero brief at .agents/skill-artifacts/mkt/short-form-brief/standup-bot/brief.md
✓ Reels variant at .agents/skill-artifacts/mkt/short-form-brief/standup-bot/variants/reels.md
