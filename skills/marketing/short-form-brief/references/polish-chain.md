...
|---|---|---|---|
| VN | founder | `vn-tone` | spoken-line section + full body |
| VN | company | `vn-tone` | full body |
...
2. Invokes the relevant polish skill (`vn-tone` or `humanize`) on that content
...
`vn-tone` polishes for the target register specified in `voc-extraction-agent`'s output: `casual / pro / bro / báo chí / semi-casual / pop-marketing`.
