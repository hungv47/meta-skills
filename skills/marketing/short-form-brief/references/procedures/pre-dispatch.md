...
1. **Latest matching `.agents/skill-artifacts/research/short-form-research/[slug].md`** from `.agents/manifest.json` (or `Glob` fallback) — **primary dependency** (Critical Gate 1).
   - Missing → emit: "No short-form-research artifact for this market. Run `short-form-research` first, or proceed with platform references only (briefs will lack current trend signals). [Run upstream / Proceed without]"
...
2. `brand/BRAND.md` + `skills-resources/experience/business.md` → infer `brand_mode`. Solo founder / personal brand → `founder`. Faceless product / company → `company`. Ambiguous → ask in Cold Start Q3.
3. `research/icp-research.md` + `skills-resources/experience/audience.md` → audience VoC, register, market.
4. `skills-resources/experience/content.md` → recent content decisions, market lock-in, prior brand_mode / production_mode selections.
...
- research artifact: .agents/skill-artifacts/research/short-form-research/[slug].md
...
**Note on cross-skill coordination:** `social-copy` (this stack's slot 1) also writes brand_mode-adjacent answers (founder/company voice). Both skills read+write `Content — brand mode` — ordering does not matter because the values agree by construction (both ask the same Q in the same words). If experience already has a value, both skills inherit it via warm-start.
...
- **No research artifact AND user declines to proceed** → NEEDS_CONTEXT, defer to `short-form-research`.
- **No BRAND.md AND brand_mode unresolvable from cold-start** → NEEDS_CONTEXT, defer to `brand-system`.
- **No ICP AND no audience hint** → NEEDS_CONTEXT, defer to `icp-research`.

## VN auto-routing (polish chain enforcement)

If `market = VN` is resolved (from research artifact, BRAND.md, or experience), the polish chain MUST include `vn-tone` per the Polish Chain table in SKILL.md (`vn-tone` on full body for company; `vn-tone` on spoken-line + full body for founder). The polish chain runs in Layer 2 post-critic; orchestrator does not need to ask the operator — auto-routing is the default. Anti-pattern row "VN market output without vn-tone" enforces this at critic gate if somehow bypassed.
