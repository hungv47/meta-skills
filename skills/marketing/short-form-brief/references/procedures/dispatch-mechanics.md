...
Previous: `short-form-research` (warm-start, soft-required) | Next: human producer / video editor / motion designer (no further skill chain at v1; `short-form-eval` MAY expand to score published posts against the hero brief in a future revision).
...
- Paid ad creative → `ad-copy`
- Per-platform research catalog (what's working right now) → `short-form-research` (this skill consumes; not produces)
- Campaign distribution strategy across many assets → `campaign-plan`
