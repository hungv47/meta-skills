...
- **BRAND.md** — origin/naming depth, real-tradeoff values, voice Do/Don't examples, 3-context tone range, tagline V/F/U (min 6/9) + competitor swap, concrete Lexicon Rules block, no copywriting scope creep, touchpoint-level emotional journey, register separation (expression not geometry), route-appropriate platform coverage, brand-book prose quality, **narrative tension gates NT-Q1 through NT-Q4** (sustainable persona, lived-not-aspirational values, specific community resonance, no exploitative pain narrative).
...
- **9d. Brand-Kit Board Spec + Image-Gen Prompts** — visual-agent emits a board *spec* (panel composition, visual mode, logo concept method, palette pinned to DESIGN.md) + ready-to-paste per-panel image-generation prompts to `brand/artboards/[board-name]/{spec.md, prompts.md}`. Operator pipes prompts to their image-gen tool of choice; renders (if produced) sit alongside as derivative artifacts. **No new brand decisions in the board** — gated by `references/brand-kit-rendering.md` and critic-agent BK-G1-G9. Use when the operator wants a premium brand-kit overview board without committing to Paper MCP or Claude Design.
...
- **Cross-stack contract:** schema changes (frontmatter fields, section headings, table column structures) require atomic update of `format-conventions.md` + every downstream caller (write-copy, write-ad, write-outreach, brief-landing-page, brief-graphic, plan-campaign, humanmaxxing, polish-vn, brief-shortform, map-user-flow) — never silently drift. **Exception — review fields:** the four review fields (`review_state` / `review_tool` / `reviewed_at` / `reviewer`) are additive and orthogonal. Downstream callers consume brand content by heading match and do not parse review fields, so adding them does NOT require touching downstream callers — only `format-conventions.md`.
- **Review:** Review-gated artifacts. Write the review frontmatter (`review_state` / `review_tool` / `reviewed_at` / `reviewer`) per [`references/_shared/reviewable-artifact-contract.md`](references/_shared/reviewable-artifact-contract.md); run the review per [`references/_shared/roughdraft-review-protocol.md`](references/_shared/roughdraft-review-protocol.md). Applied **per file**: BRAND.md and DESIGN.md are authored canonical brand-of-record — `review_state` defaults to `pending` and they carry the `## Review Gate` body block. ASSETS.md is a deterministic projection (auto-scanned/regenerated each run, not human-authored) — it carries the four review fields with `review_state: not_required` + `review_tool: none` and **no** `## Review Gate` block. `status` (skill quality gate) and `review_state` (human acceptance) are independent — `status: done` + `review_state: pending` is valid. Per-file defaults: `references/format-conventions.md` § "Frontmatter schema (all three files)".
...
Pipeline reference: [`references/anti-patterns.md`](references/anti-patterns.md) [ANTI-PATTERN]. Re-read before any artifact ships. 13 brand-system pipeline patterns + 4 cross-cutting marketing-stack patterns + 2 narrative-tension patterns + 2 brand-kit board patterns (21 total). Pipeline 1-13: aesthetics without strategy, generic values, archetype confusion, voice without examples, token soup, skipping semantic layer, mismatched bg/fg, dark mode as inversion, dispatching all agents for Quick Brand, inventing ASSETS.md rows, overwriting human markers, silently dropping rows on platform drop, round-tripping Claude Design exports. Cross-cutting 14-17: upstream context skipped → generic archetypes, cross-stack contract drift, polish-chain misroute, undeclared platforms padded. Narrative tension 18-19: persona-the-operator-can't-sustain, retrofitted hero's journey. Brand-kit board 20-21: brand decisions invented at the board layer, generic AI-glow palette default.
...
- **Domain catalogs** (loaded by agents at dispatch): `references/{brand-archetypes, brand-voice, visual-identity, token-architecture, token-templates, component-tokens, component-patterns, implementation-rules, platform-surfaces, typography-psychology, color-emotion, ai-slop-detection, paper-artboard-templates, artboard-generation, artifact-templates, assets-inventory, narrative-tension, brand-kit-rendering}.md`
- **Narrative tension** (loaded by strategy-agent + personality-agent + critic-agent): `references/narrative-tension.md` — 5 tension dimensions (curated-vs-lived / persona sustainability / progress-through-struggle / comeback arc / creator-led-vs-collective-vs-institutional) + 4 critic FAIL gates (NT-Q1 through NT-Q4)
- **Brand-kit board rendering** (loaded by visual-agent + critic-agent when Step 9d is invoked): `references/brand-kit-rendering.md` — one-board visual argument, 5 logo concept methods (M1-M5), 3×3 + 2×3 layouts, 8 visual modes, mockup discipline, image-gen prompt template, BK-G1-G9 FAIL gates
