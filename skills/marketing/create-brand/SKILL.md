...
- **Review-gated:** write the review frontmatter (`review_state` / `review_tool` / `reviewed_at` / `reviewer`) per [`references/_shared/reviewable-artifact-contract.md`](references/_shared/reviewable-artifact-contract.md); run review per [`references/_shared/roughdraft-review-protocol.md`](references/_shared/roughdraft-review-protocol.md). BRAND.md + DESIGN.md default `review_state: pending` and carry `## Review Gate` block. ASSETS.md is a deterministic projection → `review_state: not_required` + `review_tool: none`, no `## Review Gate`. `status` (skill quality) and `review_state` (human acceptance) are independent.
