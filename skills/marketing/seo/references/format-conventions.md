...
> Load this when writing or reading `.agents/skill-artifacts/mkt/seo-[mode].md`. Conventions are contract-level — schema changes break downstream `copywriting` consumption and operator action plans.
...
```
.agents/skill-artifacts/mkt/seo-[mode].md
```
...
- **needs_context** — audience or product context missing for relevance scoring; recommend running `icp-research` OR proceed with explicit scope reduction (e.g., "Technical Audit only — no audience-relevance scoring")
