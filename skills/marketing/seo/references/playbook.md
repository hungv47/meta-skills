...
- **Building or evaluating landing pages for conversion.** That's `lp-brief` (construction) or `lp-eval` (post-launch). seo is search-visibility-side; lp-brief is conversion-side. They're parallel-with peers, not substitutes.
- **Writing the actual copy for SEO-targeted pages.** That's `copywriting` — seo defines structure, keywords, and intent; copywriting writes the headlines, body, CTAs. seo defers downstream.
- **Channel mix and content calendar from market signals.** That's `campaign-plan` — seo informs which content pillars have search demand; campaign-plan decides which pillars to invest in.
...
seo is one of three search-visibility orchestrators in the marketing-skills stack (alongside `campaign-plan` for channel mix and `lp-brief` for conversion-focused page work). It's `horizontal` — invokable independently or post-campaign-plan when SEO is the active channel.

seo creates `.agents/skill-artifacts/mkt/seo-[mode].md` — the **per-mode SEO action record** that operators ship against, `copywriting` reads for page-level keyword targets, and downstream re-audits compare against (rename existing artifact to `seo-[mode].v[N].md` and create new with incremented version). Re-run triggers: quarterly for Technical Audit, monthly for AI SEO, after Google core updates, when entering new keyword territories.
