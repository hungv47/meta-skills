...
> Load when entering `/seo` Cold or Warm Start. Captures the full read order, dimensions, Cold/Warm prompts, write-back map, chain position, and IMC-coordination logic.
...
1. **Pipeline artifacts:**
   ...
   - `.agents/skill-artifacts/mkt/campaign-plan.md` → pillars + angles (informs Q1 mode + IMC coordination)
   - `research/product-context.md` → category + product context (informs Q2 site type if domain not supplied)

2. **Experience substrate:**
   - `skills-resources/experience/audience.md` → search behavior + geo + language (resolves Q3 + Q4 if previously written)
   - `skills-resources/experience/product.md` → product context (informs Q2)
   - `skills-resources/experience/business.md` → market scope (informs Q4 if geo previously declared)
...
- audience → "[from icp-research.md / experience/audience.md]"
...
| Q | File | Key |
|---|---|---|
| 3. Audience (if novel / not from icp-research) | `audience.md` | `Audience — search behavior` |
...
**VN auto-routing rule:** if Q4 declares a Vietnamese market AND the artifact will produce user-facing prose (Findings narrative, Priority Actions, Next Step), flag in the Cold Start ack: "VN-market detected — will recommend `vn-tone` polish pass on the artifact's user-facing prose before delivery." See `anti-patterns.md` § "VN-market output without vn-tone."

---

## Chain Position

- **Upstream:** `icp-research` (canonical audience), `campaign-plan` (content pillars + angles), `market-research` (competitive context)
- **Downstream:** `copywriting` (writes the actual page-level copy SEO defines), content production / site updates / engineering implementation
...
| Situation | Defer to |
|---|---|
| Production copy for comparison pages or SEO-targeted content | `copywriting` (after SEO defines structure/keywords) |
| Content pillar prioritization from market signals | `campaign-plan` (SEO supplies search-demand inputs) |
| Conversion-focused landing-page construction | `lp-brief` (marketing-skills) |
| Post-launch landing-page evaluation inside an eval-loop | `lp-eval` (marketing-skills) |

---

## Coordination with IMC Plan

When `.agents/skill-artifacts/mkt/campaign-plan.md` exists, seo and the IMC coordinate by pillar:
