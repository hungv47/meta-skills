...
1. **Gather context.** Read `research/product-context.md`, `research/icp-research.md`, `.agents/skill-artifacts/mkt/campaign-plan.md`. Identify site type, CMS/framework, known issues. Resolve all four Pre-Dispatch dimensions (mode, site, audience, geo+language).
...
8. **Apply verdict:**
   - PASS → deliver artifact at `.agents/skill-artifacts/mkt/seo-[mode].md` (rename existing to `seo-[mode].v[N].md` first if present).
