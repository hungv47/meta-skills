...
Output path: `.agents/skill-artifacts/mkt/seo-[mode].md` (mode ∈ {audit, ai, programmatic, competitor, aso}). On re-run, rename existing to `seo-[mode].v[N].md` and create new with incremented version.

Frontmatter (REQUIRED): `skill: seo`, `mode`, `version` (int), `date`, `status`.
