...
- **Need user flow mapping.** Use `user-flow` — screen-by-screen, not brand identity. user-flow consumes brand-system's design tokens and component context.
- **Need marketing copy.** Use `copywriting` — voice consumer, not voice definer. copywriting reads BRAND.md voice attributes + lexicon.
- **Need a single visual asset (social post, OG card, thumbnail).** Use `design-brief` — per-asset brief, consumes BRAND.md + DESIGN.md.
- **Need a landing page architecture.** Use `lp-brief` — page-level, consumes brand-system as ground truth.
- **Audience research missing.** Run `icp-research` FIRST — brand without audience research → generic archetypes. brand-system flags this at Pre-Dispatch when `research/product-context.md` is absent.
