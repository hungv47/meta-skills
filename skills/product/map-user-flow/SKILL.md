...
- Visual brand identity → `/create-brand`.
- Technical API design → `/architect-system`.
- Task decomposition from existing flow → `/breakdown-tasks`.
- Scoping unclear requirements → `/discover` first.
- Single-page landing surface → `/brief-landing-page`.
...
- **One flow = one file.** No pooling. Each run writes `.forsvn/artifacts/product/flow/<flow-name>.md`.
- **Stale product context (>30 days) misaligns flows.** Recommend re-running `research-icp`.
...
Run the Pre-Dispatch protocol ([`references/_shared/pre-dispatch-protocol.md`](references/_shared/pre-dispatch-protocol.md) [PROCEDURE]). map-user-flow has a **mandatory platforms+surfaces gate** before any Layer 1 dispatch — that gate sits inside Pre-Dispatch's question set.
...
Prev: `create-brand` (optional — design tokens). Next: implementation. Related: `architect-system` (API design), `breakdown-tasks` (decomposition), `discover` (specs).

**Re-run triggers:** significant feature changes, new research, usability failures, new user roles.

**Deference:** `discover` if requirements unclear · `create-brand` for design tokens · `breakdown-tasks` for decomposition.
...
Single route — all flows use the full stack. Flows >15 screens auto-split via structure-agent's sub-flow decomposition. Pipeline: Step 0 (interview + gated enumeration) → **Layer 1 parallel** (structure + edge-case) → Merge → **Layer 2a parallel** (diagram + wireframe) → **Layer 2b sequential** (validation → critic). Critic FAIL re-dispatches named agents (max 2 cycles). Deliver to `.forsvn/artifacts/product/flow/<flow-name>.md`; update `index.md` if ≥2 flows.

## Dispatch Protocol

How to spawn a sub-agent, agent conventions, and the full Layer 1 / Merge / Layer 2a / Layer 2b dispatch tables (per-agent inputs + reference files) live in [`references/dispatch-protocol.md`](references/dispatch-protocol.md) [PROCEDURE]. Layer sequence: Step 0 → Layer 1 (structure ∥ edge-case) → Merge → Layer 2a (diagram ∥ wireframe) → Layer 2b (validation → critic).
...
- **NEEDS_CONTEXT** — `research/product-context.md` missing for audience grounding; recommend `research-icp` or interview the user.
