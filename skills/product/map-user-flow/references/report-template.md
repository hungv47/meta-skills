...
- `architect-system` — reads every flow file in the directory; flows inform API endpoint design and feature decomposition.
- `breakdown-tasks` — reads flows to scope decomposition (one task per screen × surface combination as a baseline).
- `forsvn` — reads `index.md` for state detection ("flows mapped" signal).
- `review-work` (post-implementation) — reads flows to verify implementation matches the designed flow + edge states.
