...
- **Technical API design needed** — defer to `/architect-system`. Wireframe regions don't define endpoint contracts.
- **Single-page conversion surface** (landing page, ad LP) — defer to `/brief-landing-page`. user-flow rubrics are for multi-step product flows, not single-screen persuasion architectures.
