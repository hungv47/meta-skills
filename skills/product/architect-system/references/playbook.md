...
**Every dependency classified.** In-process / local-substitutable / remote-owned / true-external. This classification drives testing strategy in `breakdown-tasks` + `review-work` downstream. An unclassified dependency is an untested dependency.
...
- **v6 Phase 2 Wave 1 refactor (May 17, 2026, still v3.0.0):**
  ...
  - Chain Position narrowed: Previous now lists `/discover` OR `/map-user-flow` (baseline listed `/breakdown-tasks` as a possible predecessor — semantically incorrect since task-breakdown is downstream of architecture, not upstream; clarified in this refactor).
