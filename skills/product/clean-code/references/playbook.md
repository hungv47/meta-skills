...
**Small batches always.** Target ~30 changes per session; interim summary at 15. If each fix spawns 2+ new issues, stop and reassess scope — that's a sign the codebase needs structural redesign (use `architect-system`), not cleanup.
...
- **Standalone — no upstream gate.** `clean-code` reads only source code; no spec, no architecture artifact required. This makes it composable with any project state.
