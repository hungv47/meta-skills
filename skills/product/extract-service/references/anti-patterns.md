...
| The service that knows its caller | The service takes a `caller`/`source`/`origin` argument (or reads one from context) and branches on *who* invoked it — skips a check for the "admin" path, reshapes output for the "cli" path. Every caller's policy and trust model is now tangled into one function; a new caller means editing the service. | G7: a service treats every caller identically. Per-caller policy — auth exceptions, output shape, which steps run — is the caller's job. Pass *data* params, never caller *identity*. |
