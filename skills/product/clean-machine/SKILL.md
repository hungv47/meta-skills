...
- **Path:** `.forsvn/artifacts/meta/records/machine-cleanup-*.md` (baseline glob; report-template formalizes the date+slug convention going forward without breaking the glob)
...
- **Consumed by:** `clean-artifacts` (meta-skill — staleness signal scanning), `forsvn` (state detection), operator (history audit).
...
6. **Assembly** — compile report per [`references/report-template.md`](references/report-template.md) [PROCEDURE]. Save to `.forsvn/artifacts/meta/records/machine-cleanup-[date]-<slug>.md`.
...
8. Verify post-state.
9. Save to `.forsvn/artifacts/meta/records/machine-cleanup-[date]-<slug>.md` per [`references/report-template.md`](references/report-template.md) [PROCEDURE].
