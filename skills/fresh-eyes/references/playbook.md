...
- **Code refactoring** → `code-cleanup`. fresh-eyes verifies quality; code-cleanup changes structure. Different shapes.
- **Decision analysis with multiple perspectives** → `agents-panel`. fresh-eyes runs ONE reviewer (or 3 specialists in --thorough mode); agents-panel runs N agents debating.
