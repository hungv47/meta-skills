...
| Experience docs | `skills-resources/experience/{domain}.md` | **Append-only Q&A substrate written by every skill on cold-start, read before asking. Domains are flexible (product, audience, business, brand, goals — and any new domain skills route to). Most recent entry per dimension key wins; history kept as audit trail. See [`references/pre-dispatch-protocol.md`](references/pre-dispatch-protocol.md) for the read/write loop and per-skill question registry.** |

## Artifact Placement

The stack accumulates artifacts fast. Without a clean structure, `.agents/` and loop workspaces become junk drawers that are hard for humans AND agents to navigate. The rule below codifies a three-surface system: **infrastructure at `.agents/` root, one-shot skill outputs under `.agents/skill-artifacts/`, and measurable initiative loops under `skills-resources/loops/`**.
...
`skills-resources/` is the dedicated exception for measurable loops. Do not add other new top-level folders without clearing the canonical three-criteria test or the loop contract in `references/eval-loop-spec.md`. Folder sprawl is worse than consistent placement.

### Infrastructure placement: `.agents/` root vs `skills-resources/experience/`

Two **infrastructure** surfaces (not skill outputs):

- **`.agents/manifest.json` + `.agents/artifact-index.md`** — derived indexes of artifact metadata. Manifest is machine-readable (regenerated by `bun scripts/manifest-sync.ts`); artifact-index is the human-readable selection map. Both index `.agents/skill-artifacts/` + `skills-resources/` + canonical folders. Stay at `.agents/` root because they serve all skills equally and co-locating them inside `skill-artifacts/` would imply they're produced-by skills, which they're not.

- **`skills-resources/experience/`** — the cross-skill Q&A substrate (read by every skill on cold-start; written on cold-start answers). Lives under `skills-resources/` rather than `.agents/` because it's user-facing persistent knowledge — the user's own append-only memory of what they've told skills — not machine-derived index data. `skills-resources/` is the canonical home for shippable user-facing state (loops + experience); `.agents/` is the home for machine-derived state (manifest + index + ephemeral artifact snapshots).

Historical note: experience/ briefly lived at `.agents/experience/` (created by an early `bootstrap-experience.ts`); resolved 2026-05-16 to `skills-resources/experience/` per `implementation-roadmap/canonical-paths.md` § "experience/ — migrated."
