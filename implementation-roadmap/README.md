...
Status: **WS2 complete + validated** (2026-05-21). All 39 skills swept to "Compact + sidecar" — `SKILL.md` frontmatter ~55→~10 lines, `promptSignals` relocated to per-skill `routing.yaml` sidecars, `routing.*` dropped, descriptions rewritten to spec shape (`publish-social` un-truncated 1494→631). `hooks/skill-registry.json` byte-identical pre/post, `test-router` 25/25, `sync-skill-support --check` green. Vercel Part A scripts shipped (`scripts/explain-skill-route.ts`, `scripts/verify-counts.ts` + shared `hooks/skill-router-core.mjs`); BYO Part A applied (`produce-*` apology-language reframe); router hook stale-path bug fixed; WS3 count-drift reconciled across `plugin.json`/`marketplace.json`/`CLAUDE.md`/`README.md` (all → 39); `marketing-foundations.md` wired into its 9 consumer skills. CHANGELOG entries added under `[Unreleased]`.

**Shared-ref extractions — complete** (2026-05-21). All 3 (`confidence-labeling`, `evaluation-loop-rubric`, `production-pattern`) extracted, gated cluster-by-cluster: canonical files created, registered in `sync-skill-support.mjs`, 8 `_shared/` mirrors generated, the 8 consumer skills (research-icp/market/shortform, evaluate-ad/campaign/content, produce-asset/video) wired. Per cluster: `sync --check` green, `hooks/skill-registry.json` byte-identical, `test-router` 25/25. `confidence-labeling` extracted full per WS1 verdict + full-wired into research-shortform per operator decision; `evaluation-loop-rubric` scoped to "frame + dim contracts only" (band tables stay per-skill — they are provisional v0.1, slated for per-domain revision) per operator decision; `production-pattern` applied the same frame-vs-instrument split. CHANGELOG `[Unreleased]` entry added.

**Remaining — the operator-owned release cut:** version bump + GitHub release + retire the 4 old repos. **Note — stale roadmap instruction corrected:** `[2.0.0]` is already a cut CHANGELOG section (the 2026-05-19 consolidation) and `plugin.json`/`marketplace.json` already read `2.0.0`; the `[Unreleased]` block adds 7 skills, so the next release is a **minor bump to `2.1.0`**, not the `[Unreleased]`→`[2.0.0]` finalization earlier roadmap text described. `plugin.json`/`marketplace.json` left untouched pending the operator's version decision.
