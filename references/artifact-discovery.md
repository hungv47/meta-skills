...
1. **Every artifact is written to one predictable place** — the by-stack layered grammar `docs/forsvn/<layer>/<stack>/<name>.md`, where `<layer>` ∈ `canonical | artifacts | experience` and `<stack>` ∈ `meta | research | marketing | product`. Working output is `docs/forsvn/artifacts/<stack>/<skill>-<YYYY-MM-DD>-<slug>.md`; curated truth is `docs/forsvn/canonical/<stack>/<UPPER-NAME>.md`; memory is `docs/forsvn/experience/<stack>/<name>.md`. Path grammar: [[artifact-contract-template]] § "v3 — the by-stack layered home"; frontmatter schema: same file. (The old flat `docs/forsvn/artifacts/<stack>-<skill>-<date>-<slug>.md` grammar is legacy — still indexed for back-compat, never emitted. FORSVN's own canon lives in `docs/forsvn/canonical/`.)
