...
Practitioner-grade reference consumed by `brief-shortform` (and downstream surface-specific copy skills) to ground hooks, format compliance, algorithm fit, and anti-pattern checks. **Not generic marketing advice.** Every claim is distilled into internal operating guidance.
...
- **Definition:** Opens with a frame so information-dense (or visually labeled as a reference asset — "save this for later") that the viewer reflexively saves before the reel even ends. Exploits the save signal, which third-party cohort analysis classifies as a "heavy" interaction outweighing 4–10× as many likes.
