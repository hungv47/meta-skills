...
**Load when:** designing integrations and service boundaries (integration-agent dispatch in Layer 2). Every external dependency the architecture introduces must be classified into one of four categories. The classification drives downstream testing strategy in `breakdown-tasks` and `review-work`.
...
- `breakdown-tasks` — generates per-dependency test tasks based on category.
- `review-work` — checks that implementation matches the classification (e.g., flags when a `true-external` dependency is being tested without a boundary mock).
- `clean-code` (when refactoring) — preserves the boundary; does NOT collapse Remote but owned into In-process for "simplification."
