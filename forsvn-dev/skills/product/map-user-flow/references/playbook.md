...
- **Technical API design** — use `/architect-system`.
- **Task decomposition from an existing flow** — use `/breakdown-tasks`.
- **Scoping unclear requirements** — use `/discover` first; come back when the feature is bounded.
- **Landing-page architecture** (single-page surfaces, not multi-step product flows) — use `/brief-landing-page`.
